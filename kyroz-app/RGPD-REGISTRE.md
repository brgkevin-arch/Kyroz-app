...
| **Catégories de données** | • Identification : adresse email.<br>• **Données de santé (art. 9)** : sexe, âge, poids, taille, taux de masse grasse, niveau d'activité, sport, objectif, restrictions et préférences alimentaires.<br>• Usage : plans générés, suivi du poids, favoris, réserve alimentaire (table `pantry`). *(La série — table `streaks` — n'est plus traitée depuis le 2026-09-19 : retirée de l'app, table supprimée par `2026-09-19_drop_streaks.sql`.)* |
