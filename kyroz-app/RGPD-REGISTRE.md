...
- [ ] 🔴 🧑 **Resend : le suivi d'ouverture est-il ACTIVÉ sur le compte ?** — trouvé en
  lisant l'Exhibit A, et c'est le seul point du contrat qui puisse ouvrir un écart réel.
  Le contrat écrit que le client *« has the option to enable open/link tracking and
  other analytics/tracking of recipient actions, which could include **IP address,
  location, operating system, browser, device, email client** and spam complaints »*.
  ➡️ Si cette option est allumée, Resend collecte sur **chaque destinataire** des données
  que ni ce registre ni la politique ne déclarent — un traitement de plus, non consenti,
  sur un canal que l'utilisateur ne peut pas refuser (c'est l'e-mail d'inscription).
  ➡️ **Se mesure dans le tableau de bord, par domaine.** Tant que ce n'est pas relevé, ce
  registre ne dit rien de ces données : ni qu'elles sont collectées, ni qu'elles ne le
  sont pas. ⚠️ Défaut dormant typique — une option par défaut décide d'un traitement, et
  personne ne l'a jamais regardée.
