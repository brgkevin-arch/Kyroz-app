...
- **OTA** : la dernière est la **48ᵉ** (groupe `fada0bb6`, **iOS seulement**, 2026-09-19),
  ...
  **La 46ᵉ** (groupe `cf5f445c`, **iOS seulement**, 2026-09-19),
  ...
  différenciées, règle anti-clone R6, zéro plat principal végétal sans case cochée ;
  `ENGINE_VERSION` 58 → 60. Aucun changement natif : rien à soumettre.
  **La 45ᵉ** (groupe `419eb526`, **iOS seulement**, 2026-09-18),
  publiée sur le commit `905c310` — `main`, arbre propre.
  ✅ **Runtime iOS `823c89db…` = le build (22)**, empreinte comparée AVANT publication.
  Contenu : D43 (trois familles d'aliments évités : charcuterie, viande rouge,
  légumineuses) ; pas d'`ENGINE_VERSION`. Aucun changement natif : rien à soumettre.
  **La 44ᵉ** (groupe `65e087df`, **iOS seulement**, 2026-09-18),
  publiée sur le commit `b0dae52` — `main`, arbre propre.
  ✅ **Runtime iOS `823c89db…` = le build (22)**, empreinte comparée AVANT publication.
  Contenu : D42 (les mots des aliments évités — courgette, pommes de terre, patates,
  pomme — et la date du plan en heure locale) ; pas d'`ENGINE_VERSION`. Aucun changement
  natif : rien à soumettre à Apple.
  **La 43ᵉ** (groupe `85af0830`, **iOS seulement**, 2026-09-17),
  publiée sur le commit `ede1bda` — `main`, arbre propre.
  ✅ **Runtime iOS `823c89db…` = le build (22)**, empreinte comparée AVANT publication.
  Contenu : D41 (la liste de courses part du jour où le plan a été généré) ; pas
  d'`ENGINE_VERSION`. Aucun changement natif : rien à soumettre à Apple.
  **La 42ᵉ** (groupe `a2883e81`, **iOS seulement**, 2026-09-17),
  ...
  **La 40ᵉ** (groupe `b18c488c` pour iOS et groupe `646b20b7`
  pour Android, 2026-09-15), publiée sur le commit `085a90c` — `main`, arbre propre.
  ✅ **Runtime iOS `823c89db…` = le build (22)**, empreinte comparée AVANT publication.
  Contenu : D38 (le Catalogue montre d'abord ce que le plan peut servir, rien n'est caché) ;
  pas d'`ENGINE_VERSION`. Aucun changement natif : rien à soumettre à Apple.
  **La 39ᵉ** (groupe `7cb64449` pour iOS et groupe `bcde208a`
  ...
  **La 38ᵉ** (groupe `584093ed` pour iOS et groupe `b9bf2814`
  ...
  changement natif : rien à soumettre à Apple.
  **La 37ᵉ** (groupe `4bd24ce2` pour iOS et groupe `c37e1386`
  pour Android, 2026-09-15), publiée sur le commit `2a870c5` — `main`, arbre propre.
  ✅ **Runtime iOS `823c89db…` = le build (22)**, empreinte comparée AVANT publication.
  Contenu : D35 (ni soja texturé ni tofu chez un omnivore) ; `ENGINE_VERSION` 57, donc chaque
  plan se régénère une fois. Aucun changement natif : rien à soumettre à Apple.
  **La 36ᵉ** (groupe `d2687376` pour iOS et groupe `d416f3c5`
  pour Android, 2026-09-15), publiée sur le commit `43802b3` — `main`, arbre propre.
  ✅ **Runtime iOS `823c89db…` = le build (22)**, empreinte comparée AVANT publication.
  Contenu : D27.1 (contrôle R8 par régime) et D34 (soja texturé remplacé par les protéines
  végétales, 46 recettes réécrites) ; `ENGINE_VERSION` 56, donc chaque plan se régénère une
  fois. Aucun changement natif : rien à soumettre à Apple.
  **La 35ᵉ** (groupe `c91cc774` pour iOS et groupe `b0cbc6df`
  ...
  **La 34ᵉ** (groupe `2275a44b` pour iOS et groupe `e22e8f7c`
  ...
  plats) ; `ENGINE_VERSION` 54, donc chaque plan se régénère une fois. Aucun changement natif :
  rien à soumettre à Apple pour ces trois chantiers.
  **La 33ᵉ** (groupe `f887185e` pour iOS et groupe `13114a6e`
