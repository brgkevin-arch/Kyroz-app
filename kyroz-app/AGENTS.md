...
- ~~**E21 · On ne pouvait rien RETIRER de la liste de courses — et « terminer » gardait
  ...
  **1 300 tests verts / 76 fichiers**, `tsc` propre.
  ...
  ⚠️ **NON VÉRIFIÉ À L'ÉCRAN, et il faut le dire** : `preview_start` ne lit que le
  `launch.json` du dépôt PRINCIPAL et y résout son `cwd`, donc il sert le code du
  principal — pas celui du worktree. L'isolation du worktree interdit d'écrire dans cette
  copie de travail (c'est celle d'une autre session). Le câblage *appui long →
  confirmation → retrait* reste à voir en vrai. Contournement pour la prochaine session :
  lancer à la main depuis le worktree, `EXPO_ROUTER_APP_ROOT=$PWD/app npx expo start --web`.
