...
| Tests | **1 117 verts**, 65 fichiers · `tsc` propre | `npm test && npx tsc --noEmit` |
| Design | **4 passes livrées** — 5 onglets refaits (2026-08-03) · rayons + échelle typo sur TOUT le reste (2026-08-03) · repli du grand titre sur les 5 onglets (2026-08-04) · **écran Plan allégé + accent étendu à la barre de macros (2026-08-06)**. **Design system poussé vers Claude Design** — 6 pages GÉNÉRÉES depuis `theme.ts` (`npm run design:build`), jamais écrites à la main. Maquette de référence : `mockups/kyroz-mockup.html` — **versionnée depuis le 2026-08-06**, à la RACINE du dépôt, hors du dossier `kyroz-app/`. *(La formulation d'avant, « hors dépôt app », se lisait « hors versionnement » : le fichier est resté 3 jours sur une seule machine tout en étant cité ici comme référence.)* ⚠️ Le rayon ne se relit pas, il se **mesure** | `npm test -- rayonsDA` · `getComputedStyle` dans le panneau, cf. `docs/comparer-maquette.md` |
...
| Reroll (« Régénérer mon plan ») | 1ers repas qui changent : **repetitive 53,0 % · balanced 69,6 % · max 73,8 %** (13,7 % pour tous avant A21). ⚠️ **Re-mesuré le 2026-08-03 : la ligne annonçait 51 / 78 / 83 %, périmé depuis un moment et PAS à cause de D21-D22** — sur le catalogue d'avant ces deux chantiers, le même script rend 55,4 / 69,0 / 71,4 %. Un chiffre de cette table se re-mesure avant d'être cité | `npm run mesure:reroll` |
...
| Vivier vu par UN utilisateur | croisement gabarit × **régime** × créneau. Les cellules les plus pauvres restent vegan/vegan+SG. Pire : collation · F 55 sèche · vegan+SG = **9 recettes, 8 familles** — c'était **3 et 2** avant B7. ⚠️ Cette cellule bouge quand les TAGS bougent, sans qu'aucune recette ne change : `ciblesDe` reconstruit les cibles depuis les repas réellement servis | idem |
...
- ~~**A19 · Allègement du profil : bouclier retiré, carte de série resserrée**~~
  ...
  **au moment où ça sert**. Ce qui saute, c'est le statut permanent « Prêt » et sa
