...
| `ENGINE_VERSION` | **46** (invalide les plans en cache) — le budget du jour suit la dépense RÉELLE du jour (`lib/dailyBudget.ts`), le plan n'est plus isocalorique *(45 = `tags.objectif` rendu mécanique)*. ⚠️ Cette ligne est restée à **45** pendant une journée entière après le bump : celui qui incrémente la constante est celui qui doit toucher cette case | `lib/planEngine.ts` |
...
| Tests | **1 145 verts**, 70 fichiers · `tsc` propre | `npm test && npx tsc --noEmit` |
...
- 🔴 **A29 · Mot de passe oublié — le parcours N'EXISTAIT PAS, il est livré (2026-08-07)**
  ...
  d'écrire à `contact@kyroz.app`. Repéré en livrant A28, hors de son périmètre.
...
- 🔴 **A28 · Confirmation e-mail — CODE LIVRÉ, l'interrupteur reste à basculer (2026-08-07)**
