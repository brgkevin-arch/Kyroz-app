...
| `ENGINE_VERSION` | **58** (relu le 2026-09-17 dans `lib/planEngine.ts` — v58 = les trois correctifs de D40). *Historique :* **57** (relu le 2026-09-15 dans `lib/planEngine.ts`, dont le changelog fait foi : v57 = ni soja texturé ni tofu chez un omnivore, D35 ; v56 = soja texturé remplacé par les protéines végétales, D34 ; invalide les plans en cache). *Historique de la case, qui annonçait encore la v47 :* créneaux de repas LIBRES : l'ordre canonique de la journée devient CHRONOLOGIQUE (la collation de 16 h passe avant le dîner, elle était servie en dernier), donc le report de budget de repas en repas change d'ordre *(46 = le budget du jour suit la dépense RÉELLE du jour, `lib/dailyBudget.ts`)*. ⚠️ Cette ligne est restée à **45** pendant une journée entière après le bump : celui qui incrémente la constante est celui qui doit toucher cette case | `lib/planEngine.ts` |
