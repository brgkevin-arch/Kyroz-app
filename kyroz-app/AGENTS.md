...
| Tests | **931 verts**, 51 fichiers · `tsc` propre | `npm test && npx tsc --noEmit` |
...
- **B2 · Paywall — 🤖 L'ÉCRAN ET LE VERROU SONT POSÉS le 2026-08-01. Reste le SDK.**
  ...
  **0 occurrence** de RevenueCat — le `.web.ts` tient.
  ...
  6. 🧾 **RGPD** : rattacher l'UUID Supabase fait de RevenueCat un **sous-traitant**.
     Deux choses avant la mise en vente — le citer dans les CGU / la politique de
     confidentialité, et décider si `delete-account` doit aussi effacer chez eux
     (leur API d'effacement demande la clé SECRÈTE, donc du code serveur).
     Détail : `MONETISATION.md` §C.
