...
| Tests | **1 345 verts**, 81 fichiers · `tsc` propre — **mesuré le 2026-08-10 sur la branche E30/E31 REBASÉE sur `origin/main` (`eb71ebe`), donc après les merges de #76, #77 et #78 ; sur `main` seul : 1 334 / 80.** Nouveau fichier : `profilSection.test.ts` (11 cas, 9 mutations — le rangement de l'onglet Profil). ⚠️ Ce compte a été RE-MESURÉ deux fois dans la même session : les chiffres pris avant rebase (1 337 puis 1 340) étaient justes chez leur auteur et faux dès la fusion suivante — le défaut structurel décrit dans la ligne « Tests » du bas, rejoué. |
...
- ~~**E34 · Le rangement du Profil — lot 1**~~ ✅ **LIVRÉ le 2026-08-10.**
  ...
  **vérifié par mutation** : « Régénérer » redevenu une ligne de menu (rouge) · un champ
  ...
  📊 **1 334 / 80 → 1 345 / 81**, `tsc` vert. ⚠️ Ces deux chiffres ont été RE-MESURÉS
  après rebase : la première rédaction annonçait « 1 337 → 1 340 », mesuré sur une base
  qui ignorait quatre PR fusionnées entre-temps. Un compteur pris avant fusion périme au
  merge suivant — il se re-mesure, il ne se recopie pas.
