...
| Tests | **1 318 verts**, 78 fichiers · `tsc` propre — **les DEUX moitiés mesurées le 2026-08-09 sur le même arbre** : `origin/main` (`0ebd6ec`) rend **1 314 / 77**, cette PR (E22, le compteur d'émojis) rend **1 318 / 78**. Delta **+4 / +1**, et il tombe juste : 2 tests du nouveau fichier, 2 ajoutés à `streak.test.ts`. ⚠️ **La ligne d'avant annonçait 1 309 sur `main` seul — la mesure en rend 1 314.** Cinq tests d'écart en une journée, sans que personne ne mente : le chiffre avait été écrit avant deux merges. C'est la troisième fois que cette case dérive, toujours de la même façon. *(Historique : le 2026-08-08, 1 309 sur `main` + la PR E21, 1 281 / 75 sur `main` seul.)* ⚠️ Sur `main` seul le compte est 1 281 / 75 : ce chiffre-ci vaut APRÈS fusion, et se re-mesure au merge comme toujours. `main` contient A28, les créneaux de repas, le tuto (#44) et les courses. ⚠️ `main` annonçait 1 241 et A28 ajoute 6 tests : la somme donnerait 1 247, la mesure rend **1 248**. Encore une fois — et c'est la deuxième en deux rebases — **ce compte se re-mesure après fusion, il ne s'additionne pas**. ✅ **Les DEUX emplacements sont alignés** (ici et la ligne « Tests » de « Data / thème / qualité ») : l'autre était resté à 1 241 pendant que celui-ci disait 1 248 — le défaut décrit là-bas, rejoué le lendemain de son écriture | `npm test && npx tsc --noEmit` |
