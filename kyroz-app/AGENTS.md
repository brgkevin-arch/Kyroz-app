...
| Kyroz+ | **encaissement armé, verrou inerte.** Clé RevenueCat posée dans EAS et vérifiée dans le bundle ; `PAYWALL_LAUNCH` = `null`, donc **tout est gratuit pour tout le monde**. ✅ **4 produits chez Apple** en 2 paliers (lancement 3,99/29,99 en vente · standard 4,99/39,99 en réserve), tous attachés à l'entitlement `premium`. ✅ Le verrou `transformation` est enfin CÂBLÉ (2026-08-25) — il ne l'était nulle part, sur 3 surfaces. 🔴 Reste : **capture de review** (exige un binaire, donc le bac à sable ne peut pas passer avant le build), **captures de fiche** (aucune), bac à sable, puis la date. ⚠️ Le build TestFlight actuel est ANTÉRIEUR à la clé | `npm run check:abonnements` · `PROCEDURE-2026-08-25-…md` |
