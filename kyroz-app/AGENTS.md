...
> est faite, elle descend dans le Journal avec sa date »*. Elle n'avait plus été appliquée
> depuis des semaines : **143 fiches sur 145** décrivaient du travail LIVRÉ dans une liste
> intitulée *« ce qui reste à faire »*. Leur texte est intact, leur ordre aussi ; seule leur
> place a changé.
>
> ⚠️ **27 d'entre elles ne portaient AUCUN marqueur de clôture** — elles se lisaient comme
> des tâches ouvertes. Elles ont été reclassées **par mesure**, pas au jugé : le fichier de
> test qu'elles citent existe dans le dépôt, ou un commit de l'historique les nomme. Elles
> portent `✅ *(reclassée par mesure le 2026-08-30)*`.
>
> Les fiches **sans** cette preuve sont restées dans la liste unique : `D4-bis` (c'est une
> photo de catalogue, pas une tâche) et `E19`.
>
> 🔴 **Ce qui a permis la dérive** : rien ne compte cette liste. Un marqueur oublié la laisse
> grossir en silence, et elle grossit **dans le sens rassurant** — une fiche livrée qui traîne
> ressemble à du travail en attente, jamais l'inverse.
...
- ✅ **A41 · « TOUTES TES DONNÉES SERONT SUPPRIMÉES » N'ÉTAIT PAS VRAI — CORRIGÉ le
  2026-08-27** (constats `01-03` P1 et `03-01` P1, deux orphelins d'A39). ✅ **L'ÉTAPE
  HUMAINE EST FAITE LE MÊME JOUR** — `docs/archive/2026-08-27-procedure-suppression-revenuecat.md`
  est **close** : secret posé (au bon nom, en **v1** — deux pièges payés), fonction déployée
  en **version 8**, vérifiée sur un vrai compte jetable par **deux signaux indépendants**
  (journal muet côté Supabase, abonné absent des *Customers* côté RevenueCat), inscrite au
  registre, et les **trois abonnés orphelins** des tests d'avant retirés (3 × `200`,
  `deleted: true`). ➡️ **Le §7 de la politique est exact depuis ce jour-là**, y compris pour
  les non-abonnés — c'est-à-dire tout le monde.
...
- ✅ **A42 · LES QUATORZE ORPHELINS P2 — LIVRÉS le 2026-08-27.** Avec A40 et A41, les
  **17 orphelins de `CA-6-04` sont clos**. ⚠️ **Il ne reste plus qu'UNE étape humaine** —
  elles étaient deux à l'écriture de cette fiche, la première a été faite le jour même.
  Nommées plus bas.
  ...
  **Ce qui a changé une règle ou un outil**
  ...
  ➡️ **L'ÉTAPE HUMAINE QUI RESTE — il n'y en a plus qu'une :**
  1. ✅ ~~`docs/archive/2026-08-27-procedure-suppression-revenuecat.md` (A41)~~ — **FAITE le
     2026-08-27** : secret posé, Edge Function déployée en v8, vérifiée sur un compte
     jetable, orphelins retirés. La procédure est close et se lit comme une trace.
     **Le §7 de la politique est exact depuis ce jour-là.**
  2. 🧑 **`01-07`** — poser la clé RevenueCat Android, ou **acter par écrit qu'Android sort
     sans achat**. À trancher avant la soumission Android. *(Toujours ouvert au 2026-08-27
     au soir : l'app Android n'est rattachée à aucun projet RevenueCat.)*
...
- ✅ **A44 · SDK 57 — MONTÉ le 2026-08-27, et la ligne OTA est COUPÉE EXPRÈS**
  (demande fondateur : « SDK 57 fais ça » ; constats `04-01`, `04-05`, `03-03`).
  🔴 **LE CHIFFRAGE DISAIT VRAI, ET IL SOUS-COMPTAIT DE DEUX.** `04-01-CHIFFRAGE-SDK57.md`
  annonçait 13 dépendances déplacées ; il y en a **15** — `expo-file-system` et
  `expo-glass-effect` sont arrivés dans le dépôt après la mesure du 2026-08-26. Aucune
  ajoutée, aucune retirée, et les sept paquets annoncés inchangés le sont (`react`,
  `react-dom`, `@supabase/supabase-js`, `react-native-purchases`, `react-native-svg`,
  `react-native-safe-area-context`, `react-native-web`).

  | | avant | après |
  |---|---|---|
  | `expo` | `~56.0.12` | **`^57.0.9`** |
  | `react-native` | `0.85.3` | **`0.86.3`** (Hermes corrigé) |
  | `tsc` | 0 | **0** |
  | Tests | 2 014 / 133 | **2 017 / 134** (+3 : le garde-fou neuf) |
  | `expo-doctor` | 2 checks en échec | **21/21, « No issues detected! »** |

  ✅ **Les deux checks en échec disparaissent d'un coup** : la régression mémoire Hermes
  (`04-01`) parce que RN 0.86.3 embarque le moteur corrigé, la dérive des dix paquets
  (`04-05`) parce que `--fix` les réaligne.

  🔴 **LA MOITIÉ QUI COMPTE N'EST PAS LA MONTÉE, C'EST `runtimeVersion` — et la séparer
  aurait armé un piège.** Le chiffrage l'écrivait (« la bascule vers `fingerprint` doit se
  faire dans le MÊME lot, pas après ») ; voici pourquoi, mesuré : sous `appVersion` le
  runtime valait `expo.version`, **`1.0.0` depuis le premier commit**. Il ne pouvait donc
  dire non à personne. Une OTA publiée depuis cet arbre-ci se serait déclarée compatible
  avec **tous** les binaires `1.0.0`, dont le (6) qui est en SDK 56 — un bundle attendant
  une architecture native absente, poussé à tout le parc en minutes, sans revue pour
  l'arrêter. Ce n'est pas une dégradation : c'est une app qui ne démarre plus.
  ➡️ `fingerprint` hache la surface native elle-même : un binaire d'une autre surface ne
  correspond plus, donc **il ne reçoit rien**. La coupure devient explicite au lieu d'être
  fatale. ⚠️ **C'est la vraie raison pour laquelle « SDK 57 seul » n'existe pas** — le SDK
  coupe la ligne de toute façon ; la politique décide seulement si la coupure se voit.

  ⚠️ **CE QUE ÇA COÛTE, ET C'EST À SAVOIR AVANT DE MERGER** : le parc actuel est **figé sur
  la 25ᵉ OTA**. Les trois P0 (`ENGINE_REV` 10) et la phrase iCloud n'atteindront personne
  avant le build (7). Publier la 26ᵉ depuis un arbre SDK 56 était la seule façon de les
  donner plus tôt — c'est une décision de séquencement, pas un défaut.
  ⚠️ Et à partir de maintenant, la ligne se coupe à **chaque** changement de surface native,
  pas « une fois par version » (`CA-5-03`) : éditer `app.json` est devenu un geste de
  LIVRAISON.

  📊 **Ce qui a été vérifié, et ce qui ne l'a pas été.** Vérifié : `tsc` 0 · 2 017 tests ·
  `expo-doctor` 21/21 · **l'export web complet** (15 routes pré-rendues, 1 bundle — c'est le
  chemin qui casse en premier sur une montée, puisque le pré-rendu tourne dans Node) ·
  **les permissions résolues, identiques** (2 Android, 2 iOS, les trois bloquées toujours
  bloquées, `allowBackup: false` intact) · `check:ota` vert. **NON vérifié : le build natif
  et le comportement sur appareil** — c'est exactement ce que le chiffrage disait ne pas
  pouvoir dire, et ça ne change pas.
  ℹ️ Le premier export web a échoué sur `supabaseUrl is required` : c'est le piège documenté
  du pré-rendu sans `.env.local` (CLAUDE.md §11), pas une régression SDK 57 — l'export passe
  avec des valeurs factices. *Mesurer l'instrument avant d'accuser le changement.*

  ➡️ **Deux fichiers ont dû suivre, et le second est instructif** : `scripts/check-ota.ts`
  comparait le runtime publié au littéral `'1.0.0'` — il lit désormais la POLITIQUE et
  distingue un écart de fiche d'un état de transition (sinon le contrôle serait rouge en
  permanence jusqu'au build, donc illisible). Et le commentaire de
  `permissionsDeclarees.test.ts` disait « retrait sans effet sur la ligne OTA, la politique
  vaut `appVersion` » : **cette phrase s'est INVERSÉE**, elle est gardée et corrigée sur
  place. *Une note qui explique un mécanisme meurt le jour où le mécanisme change.*
  ➡️ Garde-fou : `lib/__tests__/ligneOta.test.ts` — **3 cas, 3 mutations, 3 rouges** (retour
  à `appVersion`, politique inventée, `expo.version` qui bouge).

  🔁 **Ce qui reste du lot 2′** : `expo-system-ui` (`03-04`) et le crash reporting (`03-05`),
  reportés par le fondateur à la semaine suivante. `03-02` était déjà livré.

- ✅ **A45 · LA DATE DE LANCEMENT DE KYROZ+ EST POSÉE, le 2026-08-27** (décision fondateur :
  « date-le à aujourd'hui »). `PAYWALL_LAUNCH = '2026-08-27T00:00:00+02:00'`. **Les deux
  interrupteurs sont allumés** — la clé RevenueCat l'était depuis le 2026-08-03.
  🔴 **ET LA POSE DE LA DATE A RÉVÉLÉ QUE L'ORDRE DE LA PROCÉDURE ÉTAIT FAUX D'UN CRAN,
  dans trois documents.** Ils disaient « la date se pose en DERNIER, après la revue ». Or
  l'écran de vente ne se rend que si `reason === 'locked'` (`kyroz-plus.tsx:90`) — donc
  seulement quand la date est posée. Sans elle, **le binaire n'a aucun écran d'achat** :
  la capture de review qu'Apple exige, qui EST cet écran, ne pouvait pas être prise, et le
  relecteur — dont le compte est créé pendant le test, donc postérieur — n'aurait trouvé
  aucun moyen d'acheter pour une app qui déclare quatre abonnements. **La date n'était pas
  la conclusion de la chaîne, elle en était le premier maillon.**
  ⚠️ **Elle n'atteint personne aujourd'hui** : elle voyage dans le bundle, et la ligne OTA
  est coupée (A44). Elle est dans le **build (7)**, fait le 2026-08-27 — reste sa distribution.
  🔴 **ELLE ARME UNE OBLIGATION QUI DORMAIT — le MÉDIATEUR de la consommation.** L'adhésion
  (L.612-1) ne vise que le professionnel qui VEND : tant que Kyroz était gratuit, elle ne
  mordait pas. Elle mord à la **première vente**. **Aucune adhésion n'existe**, et ce n'est
  pas une phrase à écrire dans les CGU — c'est un contrat à souscrire, payant ; les CGU
  devront ensuite porter son nom et ses coordonnées (L.616-1).
  ⚠️ **Le fuseau est explicite, et ce n'est pas décoratif** : `Date.parse('2026-08-27')`
  vaut minuit **UTC**, soit 02 h à Paris — les comptes créés entre minuit et 2 h ce jour-là
  seraient tombés du côté grand-péré, offerts à vie par une convention d'écriture.
  📊 **Trois garde-fous ont rougi, et c'était leur travail** : ils exigeaient
  `PAYWALL_LAUNCH === null` en disant « si ce test rougit, relire ce bloc en entier ».
  Relus, puis réécrits pour mesurer la NOUVELLE réalité — `premium.test.ts` vérifie
  désormais que la date est lisible, qu'elle porte un fuseau et qu'elle est dans le PASSÉ
  (sinon le relecteur ne voit rien) ; `identificationDifferee.test.ts` mesure la table de
  vérité complète d'`entitlementNecessaire`, dont la branche « sans date, on n'interroge
  personne » — la seule dont l'inversion enverrait l'identifiant de tout le monde.
  🔁 **VÉRIFICATION DUE AU BUILD (7), qu'aucun test ne peut rendre** : que l'identification
  RevenueCat parte bien pour un compte verrouillé. Le web ne configure aucun SDK d'achat.
  🔴 **ET LA POSE DE LA DATE A FAIT TOMBER LA CONDITION DE DEUX CONSTATS — ils étaient
  écrits « sans effet tant que `PAYWALL_LAUNCH` est `null` », ils ne le sont plus :**
  · **`01-05`** — la feuille de suppression ne dit nulle part que supprimer son compte
    **n'annule pas** l'abonnement App Store. Quelqu'un supprime en croyant arrêter le
    prélèvement, et continue d'être débité : motif de litige, et de refus. Le correctif
    est **une phrase**, avec le chemin réel (Réglages iOS → Abonnements). **Non fait, non
    demandé** — à arbitrer, et ça partira avec le build (7) si rien n'est décidé ;
  · **`09-04`** — le **médiateur** (ci-dessus). La phrase fautive a été retirée le
    2026-08-26 ; il reste l'adhésion, qui n'existe pas.
  ⚠️ *Deux constats classés « conditionnés » ont changé de statut sans que personne ne les
  rouvre : c'est un interrupteur qui déplace une liste de blocages, pas seulement un
  comportement.*

- ✅ **A46 · L'ÉCRAN « C'EST PRÊT » — une phrase, et un bouton qui rétrécissait, le
  2026-08-27** (demandes du fondateur sur capture).
  · **La phrase** en était deux, choisies par un prédicat `modulé` (les jours diffèrent-ils
    d'au moins 40 kcal ?). Elle est unique : « Ta semaine est calée sur ton objectif. » Le
    prédicat part avec elles, ainsi que l'import `baseDayTargets` qu'il seul employait.
    ⚠️ **Ce que la nuance disait n'est plus dit nulle part sur cet écran** : le « kcal »
    affiché est une MOYENNE hebdomadaire dès que des séances sont déclarées. Le mot
    « moyenne » avait quitté le libellé de colonne le 2026-08-12 pour cette phrase-là ;
    il n'a plus d'endroit. Consigné dans le fichier plutôt que perdu.
  · **Le bouton** sortait en petit pavé collé à son libellé. La cause n'est pas dans
    `PrimaryButton` : le conteneur est en `alignItems: 'center'` et le bouton ne pose ni
    largeur ni padding horizontal — il rétrécissait donc sur son texte. Ses deux blocs
    voisins portaient déjà `alignSelf: 'stretch'` ; celui-ci avait été oublié. Correction
    LOCALE : la poser dans `PrimaryButton` toucherait ses dizaines d'appelants.
  📊 **Vu à l'écran en 375 pt sur le vrai reveal**, pas en relisant le fichier. Le
...
- ✅ **A38 · SÉLECTION BMR « R6 LISSÉE » — LIVRÉE le 2026-08-24** (décision fondateur,
  ...
  **636 corps plausibles**.
  ...
  ✅ **PUBLIÉE EN OTA le 2026-08-24** — la **21ᵉ**, groupe `4d38f61c`, iOS + Android,
  runtime 1.0.0, vérifiée en tête du canal. Les cibles des testeurs bougent donc dès
  leur **deuxième** lancement (`fallbackToCacheTimeout: 0`), et l'avertissement
  one-shot de la rev 8 les explique à l'écran. Relevé de la mesure préalable : ligne
  « OTA publiées » de la table de référence — avec le piège `strings` qui s'y est
  rejoué, et qui est PIRE que ce que le dépôt en disait (un fragment ASCII pris dans
  une phrase accentuée est invisible lui aussi : c'est la chaîne ENTIÈRE qui passe en
  UTF-16, donc un témoin Hermes doit être un identifiant ou une chaîne ASCII de bout
  en bout).
  ⏳ **Reste** : (1) **aucun BINAIRE ne porte R6** — le dernier build iOS est le (6).
  Sans conséquence pour les testeurs, que l'OTA couvre ; mais le **relecteur Apple
  ouvre l'app UNE fois**, et une mise à jour ne s'applique qu'au lancement suivant,
  donc le build (7) devra être postérieur à ce commit. (2) La dette §8 du handoff
  (ordonnée 370/21,6 vs Cunningham côté sec, branche `measured` aux fortes
  adiposités, bande croissante) est documentée dans `tdee.ts` — **v2, décision
  fondateur requise**.
...
- ✅ **A35 · Le site public était MORT et rien ne l'a dit — CORRIGÉ le 2026-08-18**
  ...
  *Inscription*. Les comptes vivent chez Supabase, une reconnexion suffit — mais le
  ...
  `docs/archive/2026-08-18-procedure-domaine-legal.md`, qui est une TRACE et non une
...
- ✅ **A37 · Deux chiffres NEAT contradictoires dans ce fichier, à 1,8× d'écart —
  ...
  que dans Profil → *Sport & activité*. `lib/tdee.ts` et `../docs/briefs/2026-08-10-brief-profil-et-parametres.md`
...
- ✅ **A32 · LES 12 SILHOUETTES SONT REFAITES — 2026-08-23.** Planches générées par le
  fondateur (IA d'image, brief `../docs/briefs/2026-08-24-brief-silhouettes-masse-grasse.md`), sur fond **magenta**
...
- ~~**A29 · Confirmation e-mail**~~ ✅ **TERMINÉ, ACTIF ET ÉPROUVÉ EN PROD LE 2026-08-09** — e-mail reçu EN BOÎTE, zéro Insight Resend.
  Les 12 étapes de `docs/archive/2026-08-07-procedure-confirmation-email.md` ont été
  ...
  **⏳ CE QUI RESTE, et c'est hors du dépôt** : `docs/archive/2026-08-07-procedure-confirmation-email.md`
...
- **B2 · Paywall — ✅ TOUT EST POSÉ CÔTÉ CODE ET CÔTÉ COMPTES (2026-08-25). Reste un
  BUILD.** Écran et verrou le 2026-08-01, SDK et clé le 2026-08-02/03, verrou
  `transformation` le 2026-08-25, et les **quatre produits** existent chez Apple,
  attachés à l'entitlement `premium` chez RevenueCat.
  **Tarifs** : palier de lancement **3,99 € / 29,99 €** (en vente), standard
  **4,99 € / 39,99 €** (en réserve) — un identifiant produit PAR PALIER, jamais un prix
  changé en place (CGU §3 : tarif bloqué à la souscription).
  🔴 **Ce qui reste n'est plus du dashboard** : la **capture de review** (elle montre le
  paywall, donc elle exige un binaire), les **captures de fiche** (aucune à ce jour), le
  **bac à sable**, puis la date. Procédure pas à pas :
  `docs/procedures/PROCEDURE-2026-08-25-mise-en-vente-kyroz-plus.md`. L'état des produits se MESURE :
  `npm run check:abonnements`.
  **Rien n'est verrouillé aujourd'hui** :
  ...
  **0 occurrence** de RevenueCat — le `.web.ts` tient.
  ...
  1. ✅ **App Store Connect — FAIT le 2026-07-30** (commit `3a78fdc`) : Bundle ID,
     ...
     créés et tarifés. ✅ **DEUX AUTRES ONT ÉTÉ CRÉÉS LE 2026-08-25** (palier de
     lancement, `kyroz_plus_monthly_early` / `_yearly_early`) : ils sont quatre.
     Tous en **« Métadonnées manquantes »** — 🔴 **et c'est la CAPTURE DE REVIEW qui les
     y retient, pas les libellés** : mesuré, les descriptions fr-FR existaient depuis
     toujours et `appStoreReviewScreenshot` est vide sur les quatre. Comme cette capture
     montre le paywall, **le bac à sable ne peut pas passer avant le build**.
  ...
  ✅ **L'ÉCART DES « TROIS FORMULES » EST CLOS le 2026-08-25.** C'est le troisième MODE
  DE PAIEMENT qui est parti, pas l'écran : l'annuel payé au mois (3,99 €/mois, engagement
  12 mois) a été retiré chez Apple. Il n'avait jamais été affiché, et il serait entré en
  collision avec l'early bird mensuel — deux offres au même prix mensuel dont l'une
  engage douze mois, exactement le détail qui produit des remboursements.
  ⚠️ **L'API ne sait pas le retirer** (« Only future price changes can be deleted ») :
  interface seulement, fiche produit → Disponibilité → colonne de DROITE. ➡️ **À refuser
  DÈS la création** des prochains produits ; l'API, elle, ne le pose jamais.

  🔴 **LE NIVEAU DE GROUPE ÉTAIT À L'ENVERS, et ça ne se voyait pas.** Apple : *« Level 1
  represents the subscription that offers the most »* — le mensuel était au niveau 1 et
  l'annuel au 2, donc passer à l'annuel était traité comme une RÉTROGRADATION, différée à
  la fin du mois. Corrigé (annuels 1, mensuels 2). ⚠️ Une manipulation dans l'interface
  avait mis les deux au rang 1 : la liste affiche alors l'annuel en haut, donc l'ordre
  SEMBLE bon, alors qu'à rang égal tout changement reste différé. **Un ordre visuel n'est
  pas un rang** — seul `npm run check:abonnements` le dit.
...
- ✅ *(reclassée par mesure le 2026-08-30)* **D23 · 🤖 La vague « petits formats vegan / vegan+SG » — et elle part en OTA, PAS
...
- ✅ *(reclassée par mesure le 2026-08-30)* 🤖 **E66 · Les statistiques d'usage sont éteintes — et la garde qui compte n'est pas
  la clé (2026-08-26)**
...
- ✅ *(reclassée par mesure le 2026-08-30)* 🤖 **E65 · Les jours de repos étaient demandés deux fois, et bridés aux jours du
...
- ✅ *(reclassée par mesure le 2026-08-30)* 🤖 **E64 · La sonde des cibles tactiles s'arrêtait sur une flèche — six boutons
...
- ✅ *(reclassée par mesure le 2026-08-30)* 🤖 **E63 · Le « + » des courses — un ajout manuel ne peut pas vivre dans un cache
...
- ✅ *(reclassée par mesure le 2026-08-30)* 🤖 **E62 · Les fiches d'OTA se comptent — et le premier ancrage aurait mesuré
  une phrase morte (2026-08-26)**

  Demande du fondateur, après le défaut trouvé en publiant la 23ᵉ : `STORE-RELEASE.md`
  annonçait encore la **21ᵉ** OTA alors que la 22ᵉ était partie la veille. Rien n'avait
  rougi, parce que rien ne comptait — et c'est la fiche qu'on lit pour décider quoi
  soumettre à Apple qui décrivait un parc périmé.

  🔴 **DEUX GARDE-FOUS, PARCE QU'IL Y A DEUX PANNES DISTINCTES.** L'erreur aurait été
  d'en écrire un seul et de croire le sujet clos :
   1. **`lib/__tests__/fichesOta.test.ts`** tourne dans `npm test` et attrape la
      **DÉRIVE entre les deux copies** — le cas vécu. Rang, groupe et commit doivent
      être les mêmes des deux côtés.
   2. **`npm run check:ota`** interroge le canal `production` et attrape l'**OUBLI pur
      et simple** : deux fiches parfaitement d'accord et **fausses toutes les deux**,
      ce qui arrive dès qu'une session publie sans documenter. Le test ne peut pas le
      voir — il compare deux copies, il ne sait pas ce qui tourne.
  ⚠️ Aucun ne remplace l'autre, et le second demande le réseau, donc il ne peut pas
  vivre dans `npm test`. Même partage que `check:migrations`, pour la même raison : le
  dépôt ne sait rien de ce qui se passe hors de lui.

  🔴 **LE PREMIER ANCRAGE AURAIT ÉTÉ VERT À VIE, ET C'EST LE VRAI ENSEIGNEMENT.** Le
  geste évident est de chercher « le premier numéro de groupe du fichier ». Or
  `AGENTS.md` en porte **un autre plus haut** — `87a65d34`, dans une note datée du
  2026-08-21 que personne ne touche jamais. Le contrôle se serait verrouillé dessus :
  vert à chaque exécution, sur une phrase figée, pendant que la ligne « OTA publiées »
  dérivait juste en dessous. Il aurait eu l'air de marcher et regardé ailleurs —
  exactement le défaut d'instrument que ce dépôt paye régulièrement (`strings` sur une
  phrase accentuée, `requestAnimationFrame` dans le panneau navigateur, `espacementDA`
  devenu aveugle après la migration vers `Pressable`).
  ➡️ L'ancrage porte donc sur la **LIGNE de la table** et sur la **puce** de
  `STORE-RELEASE.md`, jamais sur le fichier. Et un cas fabriqué reproduit le piège :
  un faux `AGENTS.md` porte lui aussi un vieux groupe au-dessus de la table, et le test
  exige que ce ne soit pas celui-là qui sorte.

  ⚠️ **DEUX ANALYSEURS AURAIENT ÉTÉ LE DÉFAUT QU'ON FERME.** Le test et le script lisent
  les mêmes deux fiches : les écrire chacun de son côté aurait posé deux motifs libres
  de diverger, c'est-à-dire le problème même du chantier, en plus petit. La lecture vit
  donc dans **`lib/otaFiches.ts`**, pur et sans aucun import — même procédé que
  `collapsingTitle.ts`, `accentColor.ts` et `tours.ts` : la décision est une fonction,
  l'appelant ne fait que la rendre.

  ⚠️ **UNE FICHE ILLISIBLE EST UN DÉSACCORD, JAMAIS UN ACCORD.** Sans ça, un simple
  changement de tournure ferait rendre `null` des deux côtés et le contrôle passerait au
  vert **en ne mesurant plus rien** — le défaut que `check:migrations` évite déjà en
  refusant une liste de colonnes vide. Trois cas gardent l'entrée du test (les deux
  ancres existent, les trois champs se lisent) avant même de comparer quoi que ce soit.

  ✅ **DEUX CONTRÔLES EN PRIME, offerts par ce qu'EAS rend déjà** : `isGitWorkingTreeDirty`
  (publier depuis un arbre sale veut dire que le bundle envoyé ne correspond à AUCUN
  commit — la fiche citerait alors un commit qui n'a jamais été ce qui part) et
  l'appartenance du commit publié à `origin/main` (`git merge-base --is-ancestor`), qui
  est le contrôle « une OTA ne se publie pas depuis la branche qu'on vient de merger »,
  jusqu'ici tenu de tête.

  📋 **Vérifiés dans les deux sens, sur les VRAIES fiches.** Le test : 4 mutations —
  groupe désaccordé, rang désaccordé, ancre cassée, `AGENTS.md` avancé d'un cran — **4
  rougissements**, puis restauration verte. Le script : 2 mutations (`AGENTS.md` cite
  l'ancien groupe, `STORE-RELEASE.md` cite l'ancien commit) → **sortie 1** avec l'écart
  nommé, puis **sortie 0** une fois restauré. Relevé à froid : groupe `05baae2a`, commit
  `d71c3a2`, `android + ios`, runtime `1.0.0`, arbre propre, ancêtre d'`origin/main`.

  🔴 **ET COMPARER LA TÊTE NE SUFFISAIT PAS — objection du fondateur, le jour même**
  (*« fais en sorte que les deux soient cohérentes, non ? »*). Mesuré aussitôt : les deux
  fiches étaient bien d'accord, **et pas seulement sur la dernière OTA** — `AGENTS.md`
  cite 20 groupes, `STORE-RELEASE.md` 4, et les 4 sont un préfixe exact des 20. La moitié
  de la demande était donc déjà satisfaite. **L'autre moitié était un vrai trou** : le
  contrôle ne regardait que la tête, donc les deux fiches pouvaient diverger sur la 21ᵉ
  ou la 22ᵉ sans que rien ne proteste.
  ➡️ `chaineOta` / `chaineDivergente` comparent désormais **tout le recouvrement**, et
  l'invariant est nommé pour ce qu'il est : un **PRÉFIXE**, jamais une égalité — la fiche
  courte décide d'une soumission, la longue tient le journal, il est normal qu'elles ne
  remontent pas aussi loin. ⚠️ Une chaîne VIDE côté fiche courte n'est pas un préfixe
  valide : ce serait le contrôle qui passe au vert en ne mesurant plus rien.
  📋 Vérifié par la mutation qui compte : **la 21ᵉ changée dans `STORE-RELEASE.md` seul,
  tête intacte** → rouge, avec le rang nommé (« groupe n°3 de l'historique »). C'est
  exactement le cas que la version d'avant laissait passer.

  🚫 **CE QU'ILS NE SAURONT PAS FAIRE**, et c'est écrit dans les deux fichiers : juger
  que la PROSE autour est vraie. Ils ferment le chemin mécanique — deux copies qui
  divergent, une fiche qui décrit un canal périmé. Ils ne disent rien de ce qu'une fiche
  RACONTE de l'OTA. Comme pour `visiteGuidee.test.ts` : le mensonge reste possible, la
  dérive silencieuse non.

- ✅ *(reclassée par mesure le 2026-08-30)* 🤖 **E61 · Le tuto du Plan ne se jouait PAS certains jours — et il ne reste que
  DEUX bulles (2026-08-25)**

  Deux allers-retours avec le fondateur, en regardant les bulles tourner dans le
  navigateur. Des arbitrages de contenu — et un défaut que personne ne cherchait.

  🔴 **LE DÉFAUT : LE PLAN N'AVAIT AUCUN TUTORIEL DÈS QUE LA JOURNÉE ÉTAIT COCHÉE.**
  Mesuré dans le panneau navigateur en voulant simplement CAPTURER les bulles : quatre
  onglets sur cinq écrivaient leur marqueur `@kyroz:tour:*`, le Plan jamais — aucune
  bulle, aucune trace, à chaque visite. La sonde a montré la table de cibles VIDE au
  moment du lancement.
  La cause est une chaîne dont chaque maillon était correct :
   1. `startTour` écarte les étapes dont la cible n'est pas MONTÉE, et renonce si
      aucune ne survit (correctif du 2026-08-15, parfaitement justifié : une bulle qui
      parle d'un bouton absent est un mensonge) ;
   2. la cible du Plan vivait sur la carte du premier repas **cuisinable** ;
   3. l'auto-coche (E59) fait qu'à 23 h 59 tous les repas sont cochés — donc plus aucun
      repas cuisinable, donc plus aucune cible.
  ➡️ **Ce qui a changé, ce n'est pas le filtre, c'est l'ARITHMÉTIQUE DE LA COUPE.** Tant
  que le Plan portait six bulles, écarter celle du bouton DÉGRADAIT le tour. Avec une
  seule bulle par onglet, le même filtre le SUPPRIME. E60 a réduit le nombre de bulles
  sans se demander ce que le mécanisme d'écartement devenait à un exemplaire — le
  garde-fou d'hier, appliqué au périmètre d'aujourd'hui.
  ⚠️ **Et le commentaire de `plan.tsx` disait que c'était voulu** : « -1 quand la journée
  est entièrement mangée : […] c'est la bonne réponse ». Il l'était quand il a été écrit.
  Une justification n'est pas revérifiée quand son périmètre change.

  ✅ **Le correctif : `TourStep.targetId` devient FACULTATIF.** Sans cible, la bulle se
  pose au CENTRE, sans anneau ni trou — le rendu existait déjà (branche `!rect` de
  `Spotlight`, le filet posé le 2026-08-15 pour qu'un tuto se quitte toujours), il
  n'était atteint que par ÉCHEC de mesure. Il devient un mode assumé. Deux bulles y
  passent, à la demande du fondateur (*« enlève le cercle et mets le texte au milieu de
  la page en mode pop-up »*) : le Plan et le Profil.
  ⚠️ **Et c'est aussi le bon rendu, indépendamment du défaut** : l'anneau du Plan se
  posait sur le surtitre d'UN repas (« COLLATION · 25 MIN ») pour une phrase qui parle
  de TOUS les repas. Une bulle qui décrit l'écran ne doit désigner aucun objet.

  🔴 **UNE BULLE DE MOINS : « D'où sort ta liste » (Courses).** Elle passait pourtant le
  critère d'E60 — un article absent parce que la réserve le couvre ne laisse rien à voir
  à l'écran. Mais elle disait le mécanisme réserve ↔ liste que la bulle de la Réserve dit
  déjà dans l'AUTRE sens, et que la ligne d'aide de l'écran écrit en toutes lettres.
  ➡️ **Un critère appliqué bulle par bulle ne voit pas les doublons entre onglets** :
  deux phrases peuvent être vraies, utiles, et redondantes. C'est le fondateur qui l'a
  vu, en lisant les cinq d'affilée.
  L'écran perd aussi son « ? » : un bouton de rejeu qui n'ouvre rien est pire que pas de
  bouton, et `TOURS` étant la source unique, « Revoir les tutos » ne le propose plus.

  ✍️ **LES TEXTES SONT RÉÉCRITS** (*« on voit beaucoup que c'est toi qui parles »*). Le
  Profil disait « Enregistre ton poids et Kyroz recale calories, macros et plan » —
  l'app se citant à la troisième personne, trois noms techniques à la file. Le Plan
  décrivait un automatisme sans jamais dire que le bouton existe encore. Les deux
  partent maintenant de ce que la personne FAIT, et le Plan garde ses **deux versions**
  (réglage allumé / éteint) : chacune nomme l'autre façon de faire, aucune ne promet un
  automatisme qui n'a pas lieu — la règle d'E58.

  🧹 **NETTOYAGE QUE LA COUPE A RENDU VISIBLE.** `MealCard` portait QUATRE props
  `*TourId`, dont deux (`tourId`, `actionsTourId`) que **plus personne ne passait** —
  survivantes des bulles retirées la veille, et `actionsTourId` n'était même pas
  déstructurée. Retirées, avec `profil-poids` sur `WeightSummaryCard`.

  🛡️ **CE QUE LES GARDE-FOUS ONT ATTRAPÉ, ET CE QU'ILS ONT COÛTÉ.**
  · `harnaisEcrans.test.ts` a rougi sur le harnais Playwright, qui éteignait encore le
    tour `courses` — la liste est verrouillée contre `lib/tours.ts`, elle a fait son
    travail le jour même.
  · `profilSection.test.ts` cachait un cas **VIDE depuis la veille** : il localisait le
    bloc TDEE par `positionRendu('profil-tdee')`, un id de tuto retiré par E60.
    `positionRendu` rend `-1`, qui est bien « avant le premier bloc » → **vert quel que
    soit l'ordre réel de l'écran**. Se repérer sur l'outil d'un AUTRE mécanisme, c'est
    mourir avec lui. Il lit désormais le libellé affiché.
  · Le contrôle « les cibles d'une carte suivent l'état du repas » exigeait
    `premierCuisinable` dans `plan.tsx` : il aurait rougi sur un correctif conforme, la
    **cinquième fois** de cette famille. Généralisé à « aucun `*TourId` accroché à un
    rang fixe », dans toute l'app, avec sa sonde négative.
  · 🔴 **Une perte assumée** : « chaque forme déclarée SERT au moins une fois » est
    devenu intenable — il reste DEUX étapes ciblées pour TROIS rayons de la DA, donc le
    contrôle réclamait mécaniquement une bulle de plus. L'autre issue (retirer `carte`
    du vocabulaire) effacerait un rayon que la DA porte vraiment. Le contrôle ne compte
    plus les tokens inemployés ; il vérifie qu'aucune forme SERVIE n'échappe au moteur.
  · Deux garde-fous neufs, **vérifiés par mutation** (rouges tous les deux) : une bulle
    sans cible ne déclare aucune forme, et le filtre de `startTour` laisse passer une
    étape sans cible.

  🔁 **SECONDE PASSE, LE MÊME JOUR : DE 4 BULLES À 2, ET PLUS AUCUN « ? ».**
  · **Recettes et Réserve perdent leur tuto.** Les deux passaient le critère d'E60.
    Elles sont parties quand même, et le motif n'est plus le contenu d'une bulle mais
    leur NOMBRE : au-delà d'un certain seuil, ce qu'on arbitre est le nombre
    d'interruptions qu'on accepte de poser entre quelqu'un et son app.
  · **Le « ? » de rejeu disparaît des cinq en-têtes** (« une fois que l'user a lu,
    c'est bon, il n'a pas besoin de le revoir »). ⚠️ **Une porte devait rester** :
    un tour est marqué VU dès son OUVERTURE (seul instant qui résiste à une app tuée
    par iOS), donc une bulle ENTREVUE compte comme lue. « Revoir les tutos » dans les
    réglages du Profil est conservé — sans lui, un tuto passé par erreur serait perdu
    à vie. La justification de ce marquage nommait explicitement le rejeu comme sa
    contrepartie : retirer les deux aurait cassé un raisonnement, pas seulement un bouton.
  · **« et tu peux toujours décocher » est retiré du texte du Plan** (« ce n'est pas
    clair »). Il l'était : **il n'existe aucune case à décocher.** Le vrai geste est
    d'ouvrir le repas et de taper « Annuler » sur le bandeau « Marqué comme mangé ».
    Une bulle nomme le geste et le MOT qui est à l'écran.

  💤 **CONSÉQUENCE MESURÉE : LE CIBLAGE EST DEVENU DORMANT.** Les deux bulles restantes
  se posent au centre, donc plus personne n'appelle `useTourTarget` — et **≈ 227 lignes
  de `GuidedTour.tsx`** (mesure, défilement, anneau, trou) plus `lib/visee.ts` (85) et
  son test (108) ne s'exécutent plus jamais.
  ➡️ **Conservé à dessein**, parce que ce code porte trois correctifs durement acquis
  (le trou arrondi, la mesure jugée stable, la sortie garantie) et que le supprimer est
  une décision de produit, pas un nettoyage. **Mais la dormance est ASSÉNÉE par un test**
  (`visiteGuidee.test.ts`), sans quoi une dizaine de garde-fous de ce fichier seraient
  devenus VERTS EN NE MESURANT RIEN — le défaut d'`espacementDA`, et celui qu'on venait
  de trouver dans `profilSection`. Le jour où une bulle vise de nouveau un objet, ce cas
  rougit : c'est le signal que les autres redeviennent vivants.
  ⚠️ Et il a servi tout de suite : il a trouvé une prop `tourId` sur `MenuRow`
  (`components/ui.tsx`) que **plus aucun appelant ne passait**. Trouvée par le garde-fou,
  pas par une relecture.

  🔴 **TROISIÈME PASSE : LE BOUTON « ANNULER » DISPARAÎT — ET AVEC LUI LA RÉVERSIBILITÉ.**
  Demande du fondateur : « enlève le bouton pour annuler le marqué comme mangé ». Fait.
  Ce qu'il faut avoir en tête avant de s'en étonner ou de le rétablir :
  · **`onResetStatus` était le SEUL chemin de « mangé » vers « planifié ».** Il n'en
    existe plus aucun : un repas coché — à la main ou par l'automatisme — l'est pour
    la journée.
  · ⚠️ **Le retrait d'UN bouton a rendu FAUX TROIS textes écrits ailleurs**, dont deux
    servis à l'utilisateur : la bulle du Plan (« ouvre le repas et tape Annuler »,
    écrite deux heures plus tôt à la demande du fondateur), le texte du réglage dans le
    Profil (« Tu peux toujours le décocher ») et le commentaire qui JUSTIFIE le défaut
    allumé de l'auto-coche (« … et se décoche d'une touche »). Aucun des trois ne vit
    dans le fichier modifié. C'est la forme exacte du défaut d'E58, et rien ne l'aurait
    signalé — ni `tsc`, ni un test, ni la relecture du diff.
    ➡️ Les trois sont corrigés, et un bloc de `repasAuto.test.ts` verrouille désormais
    la DÉPENDANCE (pas le bouton) : tant qu'aucun chemin de retour n'existe, aucun texte
    ne peut en promettre un ; le jour où il revient, la sonde rougit et les textes ont
    de nouveau le droit d'en parler. **Vérifié par mutation.**
  · ⚠️ **La justification du défaut ALLUMÉ est amputée.** Elle disait « on choisit la
    panne qui se VOIT : un repas coché à tort se voit sur le plan **et se décoche d'une
    touche** ». La première moitié tient, la seconde est morte. Le défaut n'est pas
    remis en cause — c'est une décision produit — mais qui le rouvrira doit repartir de
    « visible mais irréversible ».
  · ℹ️ Et le retour arrière était **déjà partiel** : annuler ne rendait pas les
    ingrédients à la réserve (`setMealStatus` ne touche pas au stock). On a retiré un
    bouton qui ne défaisait que la moitié de ce qu'il avait l'air de défaire.

  📊 `tsc` propre, **1 718 tests verts, 111 fichiers**. Vérifié à l'écran : Plan et
  Profil en pop-up centré, aucun « ? » nulle part, Courses / Recettes / Réserve sans
  bulle, la feuille d'un repas mangé sans bouton « Annuler », et surtout **le tour du
  Plan se joue journée entièrement cochée** — le cas qui ne se jouait pas.

- ✅ *(reclassée par mesure le 2026-08-30)* 🤖 **E60 · Deux listes dans Recettes, le titre en premier, et 15 bulles de tuto en
  moins (2026-08-25)**

  Trois demandes du fondateur sur ce qu'E59 venait de livrer, plus l'arbitrage qui va
  avec.

  🔴 **« MA RÉSERVE » ÉTAIT UNE PUCE, C'EST DEVENU UNE LISTE.** E59 l'avait rangée dans
  la rangée de filtres, entre « Favoris » et « Petit-déj ». Le fondateur : *« l'onglet
  recettes, je n'aime pas, j'aimerais deux onglets séparés. »* Il a raison, et le motif
  ...
  🔴 **LE GRAND TITRE OUVRE L'ÉCRAN** (règle de DA, cf. CLAUDE.md §8). La règle d'avant
  raisonnait sur la TAILLE (« le mot reste la chose la plus grosse ») et oubliait
  l'ORDRE DE LECTURE : le compteur était petit, mais **premier**. Les trois décomptes
  sont supprimés (Réserve, Recettes, Courses), la date du Plan et le prénom du Profil
  passent SOUS le titre — ce ne sont pas des décomptes.
  ⚠️ Le fondateur a explicitement accepté de perdre « 59 aliments », « 512 recettes » et
  « 36 restants sur 37 » plutôt que de les déplacer. Celui des Courses ne coûtait rien :
  le « 1 / 37 cochés » de droite disait déjà la même chose, à l'envers.
  ➡️ Compté par `enTeteOnglets.test.ts` sur les CINQ onglets, dans les deux sens (rien
  avant le titre, et pas de sous-titre qui revient en dessous). **Vérifié par mutation.**

  🔴 **20 BULLES → 5, UNE PAR ONGLET** (« on va faire l'inventaire des tutos et on va en
  enlever les 3/4 »). Le critère appliqué, à rejouer avant d'en rajouter une : **une
  bulle ne se garde que si elle explique quelque chose d'INVISIBLE.**
  Ce qui est parti tenait dans deux familles : celles qui COMMENTENT un écran qui se lit
  tout seul (« Cocher, masquer, défaire » décrivait trois boutons libellés), et celles
  qui RÉPÈTENT une phrase déjà affichée (« Un article, deux gestes » redisait mot pour
  mot la ligne d'aide posée douze pixels plus bas ; « Le frais et le sec » redisait
  « Touche une quantité pour la modifier »).
  ⚠️ **Le coût d'une bulle de trop n'est pas le temps qu'elle prend** : c'est qu'elle
  fait passer les autres pour du décor. Vingt interruptions modales apprennent qu'on peut
  toutes les passer sans rien perdre — y compris celle qui, elle, disait quelque chose.
  ➡️ Restent : Plan « Ils se cochent tout seuls » · Recettes « Deux listes, deux
  questions » · Courses « D'où sort ta liste » · Réserve « Dis ce que tu as » · Profil
  « Ta pesée pilote tout ».
  ⚠️ **Le tour du Plan a DEUX versions d'une même bulle** : auto-coche allumée, le sujet
  est l'automatisme ; éteinte, c'est le bouton « J'ai cuisiné ». Sans ça, couper le
  réglage laissait un onglet avec un « ? » qui n'ouvre rien — et servir la version
  automatique à qui l'a coupée serait le mensonge d'E58.
  ⚠️ **Une bulle retirée emporte SA CIBLE** : les `useTourTarget` orphelins sont partis
  des cinq écrans dans le même commit. Une cible que plus aucune étape ne vise se relit
  comme une bulle perdue en route.

  🔴 **ET UNE BULLE SUPPRIMÉE PEUT EMPORTER UNE PREUVE CITÉE AILLEURS.** `METRICS.md` §2
  s'appuyait sur la bulle `plan-serie` (« elle avance dès que tu ouvres ton plan, cuisiné
  ou pas ») pour affirmer que la règle de la série était ANNONCÉE à l'utilisateur.
  `metrics.test.ts` a rougi le jour même. La page dit désormais que **l'app ne le dit
  plus** — la règle reste vraie dans le code, la pastille « 1 j de série » ne dit pas ce
  qu'elle compte, et c'est à rouvrir si la question remonte.
  ➡️ *Un document qui cite une surface d'interface comme preuve doit être compté contre
...
- ✅ *(reclassée par mesure le 2026-08-30)* 🤖 **E59 · Le frigo devient la RÉSERVE — et cinq règles changent ensemble (2026-08-24)**
...
- ✅ *(reclassée par mesure le 2026-08-30)* 🤖 **E58 · Le frigo n'est plus soustrait par défaut — et TROIS phrases promettaient
...
- ✅ *(reclassée par mesure le 2026-08-30)* 🤖 **E57 · Deux événements de diagnostic — et l'ID de recette qu'ils ne portent PAS
  (2026-08-21)**
...
- ✅ *(reclassée par mesure le 2026-08-30)* 🤖 **E56 · Le doc produit décrivait une app qui n'existe plus — il devient VIVANT
  (2026-08-21)**
...
- ✅ *(reclassée par mesure le 2026-08-30)* 🤖 **E54 · Le refus de la sèche était un mur au 7ᵉ tap — il devient une bifurcation
  (2026-08-20)**
...
- ✅ *(reclassée par mesure le 2026-08-30)* 🤖 **E51 · Les transitions : six endroits où l'app téléportait (2026-08-15)**
...
- ✅ *(reclassée par mesure le 2026-08-30)* 🤖 **E50 · Le tuto désignait l'objet d'une AUTRE bulle (2026-08-15)**
...
- ✅ *(reclassée par mesure le 2026-08-30)* 🤖 **E49 · La feuille du suivi du poids : trois choses au lieu d'une (2026-08-14)**
...
- ✅ *(reclassée par mesure le 2026-08-30)* 🤖 **E48 · Le Profil : la série s'efface, le poids prend la place (2026-08-14)**
...
- ✅ *(reclassée par mesure le 2026-08-30)* 🤖 **E47 · « Cuisiné » depuis le Frigo cuisinait ce qu'on n'avait pas choisi (2026-08-14)**
...
- ✅ *(reclassée par mesure le 2026-08-30)* 🤖 **E46 · Trois listes qui étaient des murs — le Frigo se replie, les recettes se dévoilent (2026-08-14)**
...
- 🤖 **E45 · L'app se fige sur « Rien à acheter » — UNE CAUSE STRUCTURELLE TROUVÉE ET
  CORRIGÉE le 2026-08-23** *(la fiche d'origine, écrite le 2026-08-14, suit sous ce bloc
  et reste vraie : rien de ce qu'elle a écarté n'est rouvert)*

  🔴 **`courses.tsx` PERDAIT L'INSTANCE DE SA FEUILLE À CHAQUE BASCULE D'ÉTAT, et c'est
  le seul écran de l'app dans ce cas.** L'écran a deux rendus — liste pleine et « Rien à
  acheter » — et il posait sa feuille d'historique à des positions différentes sous le
  même `SafeAreaView` : **dernier des DEUX** enfants dans l'état vide, **dernier des
  TROIS** dans la liste (`SectionList` + `CompactTitleBar` la précèdent). React
  réconcilie des enfants sans `key` **par INDEX** : à la bascule, la `Sheet` de l'index 2
  était détruite et une neuve montée à l'index 1. Donc sa `Modal` — celle d'UIKit —
  **détruite et recréée**.

  ⚠️ **Et la bascule tombe exactement dans la fenêtre d'une AUTRE transition modale.**
  « Courses terminées » ouvre d'abord un `choose` (« X articles ne sont pas cochés ») ;
  la promesse résout au choix, mais la feuille de dialogue garde sa `Modal` montée le
  temps de sa sortie. Pendant ces ~300 ms, `terminer()` enchaîne `recordTrip` →
  `removeItem` → `load()`, la liste devient vide, et l'écran change de branche. **Deux
  transitions modales dans la même frame**, ce qu'iOS ne garantit pas — c'est la moitié
  native du piège déjà consigné en CLAUDE.md §11.

  ⚠️ **LE COMMENTAIRE DU FICHIER AFFIRMAIT DÉJÀ L'INVARIANT** — « une seule définition de
  la feuille, montée par les DEUX rendus ». Il était sincère et faux : une seule
  *définition* ne fait pas une seule *instance*, et **rien ne comptait la différence**.
  Même famille que « règle écrite ≠ règle mesurée ».

  ✅ **Le correctif est structurel, pas un durcissement.** Une enveloppe unique
  (`const ecran = (corps) => …`) rend la feuille **une seule fois dans le fichier**, donc
  forcément à la même position. ⚠️ C'est une **fonction appelée**, jamais un composant
  (`<Ecran corps={…} />`) : un composant défini dans le corps du rendu change d'identité à
  chaque rendu, donc remonterait tout l'écran à chaque frappe — le défaut réintroduit en
  pire, par la porte d'à côté.

  ⚠️ **CE QUE ÇA NE PROUVE PAS, ET IL FAUT LE LIRE.** Le gel n'a toujours pas été
  reproduit : personne n'a vu ce remontage produire l'écran figé. Ce qui a changé, c'est
  que le défaut, lui, est **démontré indépendamment du symptôme** — la feuille perdait son
  instance, ça se vérifie sans hypothèse, et une feuille OUVERTE au moment de la bascule
  se refermait toute seule. C'est ce qui le distingue des deux durcissements écartés le
  2026-08-14, qui ne corrigeaient aucun défaut établi. ➡️ **Le filet de 1,5 s reste**, et
  la fiche ne se clôt pas.

  🔴 **ET LE SECOND SIGNALEMENT N'EST PAS EXPLIQUÉ** (« freeze en fermant la feuille du
  suivi du poids »). La feuille du poids vit dans `plan.tsx` et `profil.tsx`, et **ni l'un
  ni l'autre n'a ce défaut** : balayage des 15 fichiers qui montent une feuille, un seul
  retour anticipé ailleurs (`plan.tsx`, `loading`) et ses feuilles ne vivent que dans
  l'autre branche, avec un `setLoading(false)` appelé **une fois**, avant qu'aucune
  feuille ne puisse être ouverte. ➡️ Le geste de diagnostic ci-dessous reste valable, et
  c'est toujours lui qu'il faut jouer à la prochaine occurrence.

  ➡️ Garde-fou : `lib/__tests__/feuilles.test.ts` → « E45 — la feuille des Courses garde
  son INSTANCE », **vérifié par 3 mutations** (la branche vide reprend son enveloppe · la
  feuille rendue deux fois · l'enveloppe devenue composant). ⚠️ Il écarte les commentaires
  avant de compter — la note de `courses.tsx` cite `<Ecran corps={…} />` pour dire de ne
  PAS l'écrire, et la première version du test accusait la note qui met en garde.
  **1 642 tests verts, `tsc` propre.**
  ⚠️ **Non vérifié à l'écran**, et voici pourquoi : le changement est purement structurel
  (le JSX des deux branches est identique à l'enveloppe près, ce que `tsc` couvre), et un
  worktree ne peut pas servir son propre code au preview (CLAUDE.md §11). Le seul contrôle
  qui vaudrait est le simulateur, sur un chemin dont on sait qu'il ne se rejoue pas.

- 🔴 **E45 (fiche d'origine) · OUVERT — l'app se fige sur « Rien à acheter », non reproduite (2026-08-14)**
...
- ✅ *(reclassée par mesure le 2026-08-30)* 🤖 **E44 · Les trois finitions — et un SEPTIÈME geste mort trouvé en les vérifiant (2026-08-14)**
...
- ✅ *(reclassée par mesure le 2026-08-30)* 🤖 **E42 · Revue page par page du Plan et des Courses — et CINQ gestes morts en natif (2026-08-14)**
...
- ✅ *(reclassée par mesure le 2026-08-30)* 🤖 **E41 · Revue page par page de l'onboarding, du reveal et du tuto (2026-08-12)**
...
- ✅ *(reclassée par mesure le 2026-08-30)* 🤖 **E40 · La page « Méthodologie & sources » — et pourquoi elle ne contient aucun chiffre (2026-08-11)**
...
- ✅ *(reclassée par mesure le 2026-08-30)* 🤖 **E39 · Le portail de dépistage santé cesse de dépister — et un sous-traitant manquait au registre (2026-08-11)**
...
- 🤖 **E38 · Les notifications : proposées, increvables, et elles mènent quelque part (2026-08-11)**
  ...
  **vérifiés par 9 mutations**. ⚠️ **Et la 9ᵉ a rougi le test, pas le code** : le cas qui
  ...
  la feuille de pesée.
- ✅ *(reclassée par mesure le 2026-08-30)* 🤖 **E37 · Le RETOUR AU TOUCHER, et une affirmation démentie par la mesure (2026-08-11)**
...
- ✅ *(reclassée par mesure le 2026-08-30)* 🤖 **E36 · Le MATÉRIAU — la barre d'onglets passe au verre, et une prémisse tombe (2026-08-11)**
...
- ✅ *(reclassée par mesure le 2026-08-30)* 🤖 **E35 · Le MOUVEMENT reçoit son rôle, ses tokens et son compteur (2026-08-10)**
...
- ~~**E26 · Le consentement analytics gâchait l'écran d'arrivée, et mesurait des données de santé**~~ ✅ **LIVRÉ le 2026-08-10.**
  ...
  **25 ms**.
  ...
  ✅ **LOT CLOS LE 2026-08-18 — les textes, les trois verrous ET la clé** (l'ordre s'est
  inversé pour une raison : l'app demandait déjà le consentement en production pour un outil
  que les textes déclaraient inexistant ; ce n'était plus « anticiper », c'était corriger un
  énoncé faux) :
  - [x] `constants/legal.ts` — §2, §3, §4, §5, §7, §9 : PostHog nommé, stockage à Francfort,
        consentement distinct et refusable, conservation « au moins un an, sans limite haute
        fixe » (cf. le verrou rétention plus bas). Écrit au **conditionnel de consentement**
  ...
  - [x] 🔴 **`docs/politique-confidentialite-kyroz.md` — la QUATRIÈME surface, que le §8 de la
        synthèse ne comptait pas.** Elle n'affirmait rien de faux (elle ne parlait pas du tout
        d'analytics), donc elle échappait à toute recherche de la phrase à corriger — c'était
        un **manque**, pas un mensonge, et un manque ne se grep pas. ➡️ Réglé par la
        GÉNÉRATION, pas par une relecture : elle sort désormais de `constants/legal.ts` comme
        les autres, donc le paragraphe ne peut plus y manquer. *(La leçon reste : un
        inventaire juste sur le mauvais périmètre — le §8 avait listé les trois textes qui
        PORTAIENT la phrase, pas les surfaces qui la DEVRAIENT.)*
  - [x] côté PostHog, **les trois verrous levés le 2026-08-18** : IP déjà écartée par défaut
        sur les projets Cloud EU (`Discard client IP data` vérifié en console — c'était une
        vérification, pas une action) ; DPA signé, contresigné **et lu** le jour même ;
        rétention **réécrite plutôt qu'automatisée** — PostHog n'offre aucune purge
        configurable, donc « 18 mois puis suppression » ne pouvait pas être tenu.
  ...
  📄 `docs/archive/2026-08-10-brief-analytics-perimetre.md` (le brief) et
  `docs/archive/2026-08-10-synthese-analytics-arbitrage.md` (l'arbitrage qui fait foi).
...
- ~~**E23 · Katch-McArdle prenait une DEVINETTE pour une mesure**~~ ✅ **LIVRÉ
  ...
  **Écart mesuré sur les 12 silhouettes du sélecteur** : TDEE **−217 à +363 kcal/j**,
  ...
  profil morte en silence. Procédure : `docs/archive/2026-08-06-procedure-body-fat-source.md`.
...
- ~~**P3.6 — PostHog** : câblé + dormant, il manque la clé.~~ ✅ **ACTIVÉ le 2026-08-18** —
  ...
  **la clé ET les nouveaux textes**, ensemble — ce que la règle du dépôt exige.
  ...
  Détail complet : `RGPD-REGISTRE.md`, `docs/archive/2026-08-18-procedure-activation-posthog.md` §6.
