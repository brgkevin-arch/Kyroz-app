...
| `PRODUIT.md` | **Le produit vu par l'utilisateur** (2026-08-21) — les étapes de l'inscription, les cinq écrans, le ton, gratuit/payant, et **ce qui est éteint**. Autonome : c'est LUI qu'on colle dans un brief ou une fiche store, jamais un doc daté de `docs/`. ⚠️ Il remplace `docs/2026-08-15-synthese-…` (archivé), faux sur **6 sections sur 9** en cinq jours. Toute fonction retirée ou renommée y passe le jour même — `produitDoc.test.ts` compte ce qui est vérifiable. |
...
|---|---|---|
| Catalogue | **559 recettes** — 137 petits-déj · 302 repas complets · 120 collations (relu le 2026-09-19, après B13 porc ; 549 au relevé précédent) | `npm run mesure:couverture` |
| `ENGINE_VERSION` | **62** (2026-09-19, parts de protéines par régime : ce qui est coché devient une part garantie des déjeuners et dîners). *Historique :* **61** (2026-09-19, vague B13 : 10 plats au filet de porc). *Historique :* **60** (2026-09-19 : la famille voit le fruit et le goût ; sept recettes différenciées ; zéro plat principal végétal pour qui mange de la viande sans « Végétal » coché). *Historique :* **59** (2026-09-18, vague B12 : 4 repas complets sur les pièces végétales, `lardons_vegetaux` rangé dans la case Végétal). *Historique :* **58** (v58 = les trois correctifs de D40) ; **57** (relu le 2026-09-15 dans `lib/planEngine.ts`, dont le changelog fait foi : v57 = ni soja texturé ni tofu chez un omnivore, D35 ; v56 = soja texturé remplacé par les protéines végétales, D34 ; invalide les plans en cache). *Historique de la case, qui annonçait encore la v47 :* créneaux de repas LIBRES : l'ordre canonique de la journée devient CHRONOLOGIQUE (la collation de 16 h passe avant le dîner, elle était servie en dernier), donc le report de budget de repas en repas change d'ordre *(46 = le budget du jour suit la dépense RÉELLE du jour, `lib/dailyBudget.ts`)*. ⚠️ Cette ligne est restée à **45** pendant une journée entière après le bump : celui qui incrémente la constante est celui qui doit toucher cette case | `lib/planEngine.ts` |
...
| Migrations Supabase | 🔴 **UNE EST À JOUER : `2026-09-20_profiles_weigh_in_day.sql`** (E70) — sans elle, `weigh_in_day` étant dans `PROFILE_COLS`, l'upsert du profil est rejeté ENTIER et TOUTE la synchro meurt en silence. ⚠️ **Non mesuré contre la prod** : le `.env` de ce worktree pointe un hôte factice, donc `npm run check:migrations` n'y tourne pas — relancer depuis le dépôt principal avant de conclure. **20 fichiers** dans `supabase/migrations/` au 2026-09-20 (`drop_streaks` jouée le 2026-09-19, #332). *Historique :* les **18** jouées — re-compté le 2026-08-15 (`ls supabase/migrations/*.sql`) et re-mesuré contre la prod (`✅ toutes reflétées`). ⚠️ Cette case annonçait **17** : la 18ᵉ (`2026-08-10_profiles_deficit_weeks.sql`, la pause à la maintenance) était jouée depuis cinq jours sans que la ligne bouge. *Un compteur écrit à la main périme sans que rien ne le dise.* Dont `2026-08-07_profiles_meal_slots.sql` comprise (jouée le 2026-08-07 AVANT le merge de la PR #46, mesurée avant ET après : 400 → 200 ; re-mesurée le 2026-08-08, toujours 200). Les 39 colonnes de `PROFILE_COLS` passent en une requête. ⚠️ **Ne jamais annoncer une migration « en attente » sans lancer la commande** — le dépôt ne sait rien de la prod, et deux lignes d'ici l'ont dit à tort pendant des jours | `npm run check:migrations` |
| Variété perçue | semaines servant 2 recettes d'une même FAMILLE : **max 7,1 %** — re-mesuré le 2026-09-19 sur 240 semaines. ⚠️ **Définition changée le 2026-09-18** : la famille est `familyKey` du moteur, IMPORTÉE par `mesure-variete.ts` (elle était recopiée et n'avait jamais reçu la décision du 2026-09-14) — protéine × féculent, ou × fruit sans féculent, et le goût. Même jour, ancienne clé : 22,1 %. | `npm run mesure:variete` |
| Variété perçue **par régime** | **vegan+SG 16,7 %** · vegan 6,3 % — re-mesuré le 2026-09-19 (ancienne clé, 2026-09-15 : 52,1 % · 25,0 %). | `mesure:variete -- --regime=…` |
| Premier plan servi | plan **canonique** (seed 0) : **11,7 %** de semaines avec quasi-doublon — re-mesuré le 2026-09-19 (ancienne clé, 2026-09-15 : 33,3 %). | `mesure:variete -- --seeds=0` |
...
| Reroll (« Régénérer mon plan ») | 1ers repas qui changent : **repetitive 51,8 % · balanced 80,4 % · max 85,7 %** — re-mesuré le 2026-09-15 après D34, identique avant D34 au dixième près (D34 ne bouge pas le reroll). *Historique :* repetitive 63,1 % · balanced 76,8 % · max 79,8 % — re-mesuré le 2026-08-07 (la ligne annonçait 53,0 / 69,6 / 73,8 % ; 13,7 % pour tous avant A21). ⚠️ **Elle avait DÉJÀ été re-mesurée le 2026-08-03 et elle a redérivé en quatre jours.** Un chiffre de cette table se re-mesure avant d'être cité — celui-ci deux fois plutôt qu'une | `npm run mesure:reroll` |
| Anti-doublons | R1 **64** · R2 **59** · R4 14 · R5 **0** · R7 0 — relu le 2026-09-15 dans `doublons.test.ts` après D34 (R5 16 → 0 par D31 ; R2 71 → 70 par D33 ; R1 74 → 64 et R2 70 → 59 par D34). *Historique :* R1 **74** · R2 **71** · R4 14 · R5 **16** · R7 0 — **figés par un test** (`PLAFOND` dans `doublons.test.ts`), resserrés au constaté le 2026-08-03 puis déplacés par P3.4 le 2026-08-05. ⚠️ Cette ligne a annoncé « R1 81 · R2 70 » pendant deux jours alors que le TEST, lui, disait déjà 74/71 : le plafond du test est la source, pas cette case | `npm run check:doublons` |
| Drapeaux bloquants sur les repas SERVIS | **26 sur 6 720** (240 semaines : 12 profils × 5 régimes × 4 tirages) — re-mesuré le 2026-09-19, après B12, les sept recettes différenciées et le zéro végétal sans case cochée (33 le 2026-09-15). | `npm run mesure:variete` |
| Sous le seuil R8 | ⚠️ petit-déj **39/137** · repas complets **95/284** · collation **5/120** = **139** — re-mesuré le 2026-09-15 après D34, **avec la règle D27.1** (une recette sans viande jugée sur le pire des régimes sans viande qui la reçoivent). ⚠️ **Ce n'est plus la même mesure** : les chiffres d'avant le 2026-09-15 ne se comparent pas à celui-ci. Même règle, catalogue d'avant D34 : 39 / 97 / 5. *Historique :* petit-déj 36/122 · repas complets 69/280 · collation 4/110 = 109 — re-mesuré le 2026-08-07 (37 / 70 / 5 avant) ; B7 (30), B8 (8) et B9 (8) livrées, catalogue **512** | `npm run mesure:seuils` |
| Ce que ça coûte au SERVICE | **1 707 repas servis sur 10 752 (15,9 %)** viennent d'une recette sous le seuil — re-mesuré le 2026-09-15 après D34, **règle D27.1** (le seuil ne se juge plus sur les mêmes cibles : ne se compare pas aux relevés d'août). Avant D34, même règle : 1 597 (14,9 %) — D34 coûte 1 point. *Historique :* 688 repas servis sur 10 752 (6,4 %) — re-mesuré le 2026-08-07 (675, 6,3 % avant) ; **935 (8,7 %) avant B7**, 633 (5,9 %) avant D20, 611 (5,7 %) avant D21. ⚠️ **D21 a fait BAISSER le nombre de recettes sous le seuil (115 → 111 → 109) et MONTER le nombre de repas qui en viennent** : les rescapées sont servies plus souvent. Les deux chiffres disent des choses différentes, ne pas lire l'un pour l'autre. ⚠️ B9 l'avait fait remonter depuis 5,3 % : un format volontairement étroit fait BAISSER une moyenne par recette tout en améliorant la couverture réelle | `npm run mesure:vivier` |
| Vivier vu par UN utilisateur | croisement gabarit × **régime** × créneau. Les cellules les plus pauvres restent vegan+SG. **Pire au 2026-09-15 après D34 : repas complet · H 110 masse · vegan+SG = 13 recettes, 12 familles** (18 et 17 avant D34 : ⚠️ D34 en retire 5, dont les recettes sorties du sans gluten) ; puis petit-déj H 110 masse vegan+SG 14/12, collation H 110 masse vegan+SG 18/16. *Historique :* pire collation · F 55 sèche · vegan+SG = **8 recettes, 7 familles** — c'était 9 et 8 le 2026-08-06, et **3 et 2** avant B7. Juste derrière : repas complet · F 55 sèche · vegan+SG = **11 recettes**. ⚠️ Cette cellule bouge quand les TAGS ou les CIBLES bougent, sans qu'aucune recette ne change : `ciblesDe` reconstruit les cibles depuis les repas réellement servis — et depuis le 2026-08-06 ces cibles diffèrent d'un jour à l'autre | idem |
| Moyenne R8 par créneau | petit-déj **8,77/12** · repas complets **8,36** · collation **6,71** — re-mesuré le 2026-09-15 après D34, règle D27.1 (même règle, catalogue d'avant D34 : 8,77 / 8,31 / 6,91 ; ne se compare pas aux relevés antérieurs). *Historique :* petit-déj 8,84/12 · repas complets 8,85 · collation 6,80 — re-mesuré le 2026-08-07 (8,54 / 8,72 / 7,00 avant · 8,67 / 6,91 avant D22 · 8,55 / 8,61 / 6,72 avant D21 · 8,27 / 8,51 / 7,38 avant B7 · 7,62 avant D18 · 7,19 avant B6 · 4,52 avant B5). ⚠️ Les deux premiers MONTENT et la collation BAISSE : la répartition par volume a écarté les cibles des jours, ce qui sert les créneaux à large enveloppe et resserre celui qui n'en a pas | idem |
| Vegan | petit-déj **56/137** · repas **93/284** · collation **66/120** — comptage CATALOGUE (`restrictions_ok`), recompté le 2026-09-15 après D34 (56/122 · 89/280 · 67/110 au relevé précédent) | compter `restrictions_ok` par créneau |
...
- 📓 **HISTORIQUE (clos le 2026-09-12, cf. la puce juste au-dessus) · C-REJET-20 · RÉPONDRE AU REJET APPLE DU 2026-09-10** (soumission
...
- ⏭️ **RESTE À FAIRE — catalogue et moteur, relevé le 2026-09-14** (à lire avant D32 → D29)
  ...
  2. ✅ **D33 · Pas de remplissage dans les recettes — mergé le 2026-09-15** (PR #284) et
     publié dans la 35ᵉ OTA le même jour (point 4). Ce point tient lieu de fiche. 104 recettes réécrites (ingrédients, étapes, nom quand le
  ...
  3 bis. ✅ **D35 · Ni soja texturé ni tofu chez un omnivore — 2026-09-15** (décision fondateur :
     *« je ne veux pas de PST ou TOFU chez un omnivore »*). Ce point tient lieu de fiche. MUR DUR
     dans `lib/planEngine.ts::recipeAllowed` (`SOJA_TOFU_REFS`, `mangeDeLaViande`) : il vaut pour
     le plan, « Remplacer ce repas » et la liste « Réalisable » (`pantry.ts::cookableRecipes`),
     y compris quand « Végétal » est coché. Le pescétarien, le végétarien et le végane ne sont pas
     visés ; le tempeh et les protéines végétales de la liste restent servis à l'omnivore.
     `ENGINE_VERSION` 56 → 57. **Mesuré avant** (12 profils × 5 régimes omnivores × 4 tirages) :
     40 recettes concernées (9 petits-déj, 24 repas, 7 collations) ; un omnivore en recevait
     2,4–2,5 % de ses repas, **7,9–8,3 % avec « Végétal » coché** — la mise en retrait (v50) et le
     plafond D29 ne l'empêchaient pas. **Après : 0.** Coût mesuré : quasi-doublons 22,1 → 22,5 %,
     drapeaux bloquants 30 → 33, écart calorique du jour 0,70 → 0,71 %. Garde-fou
  ...
  3 quater. ✅ **D37 · « Aliments à éviter » propose les sortes du catalogue — 2026-09-15**
     (décisions fondateur, à la suite d'un retour : une personne sans gluten avait écrit « tofu » et
     en voyait encore). **Mesuré sur ses captures, le champ n'était pas en cause** : son mot était
     enregistré, et son plan (même profil, 420 repas sur 15 semaines) ne contenait ni tofu, ni
     quinoa, ni polenta. Le tofu venait de la liste « Catalogue » de l'onglet Recettes, qui lui
     montrait 291 recettes sur 541 que son plan ne lui servira jamais (200 au gluten, 42 à la
     polenta, 39 au tofu, 36 au quinoa).
     **Tranché** : (1) *« On change rien au catalogue, si la personne veut voir une recette au
     tofu, elle fait ce qu'elle veut »* — le Catalogue reste **sans filtre** (« Réalisable »
     filtre toujours) ; ne pas re-proposer de le filtrer. (2) *« S'il y a écrit tofu, il doit être
     proposé tous les mots commençant par TOFU qui sont répertoriés. »*
     **Code** : `lib/avoidance.ts::suggestionsAliments` — noms tirés des RECETTES (jamais un mot
     sans effet), même règle de début de mot que le moteur (« œuf » ne propose pas le bœuf), dès
     2 lettres, sans ce qu'un mot déjà enregistré couvre ; `DislikedFoodsField` les montre en
     bulles sous le champ, toucher une sorte l'enregistre seule, Entrée garde le mot tapé.
     ⚠️ **Choisir UNE sorte laisse passer les autres** (« tofu fumé » n'écarte que ses 7 recettes,
     « tofu » en écarte 39) : c'est l'objet de la précision, mais c'est aussi un piège, et la
     phrase « « tofu » écarte 39 recettes » reste affichée sous les suggestions pour le dire.
     Garde-fou `lib/__tests__/suggestionsAliments.test.ts` (7 cas, **vérifié par 5 mutations**).
     Vérifié à l'aperçu web (inscription, étape 6). Pas d'`ENGINE_VERSION`.
     ✅ **CORRIGÉ le 2026-09-17 (D40, point 1)** : « tofus » et « le tofu » écartent désormais ce que « tofu » écarte.
  3 quinquies. ✅ **D38 · Le Catalogue montre d'abord ce que l'on mange — 2026-09-15** (décision
     fondateur, qui PRÉCISE D37 : *« le catalogue devrait montrer en priorité les recettes avec les
     aliments qu'on mange et pas ceux qu'on refuse de voir ; sans pour autant les rendre
     invisibles »*). Le Catalogue reste **sans filtre**, il est **rangé**.
     **Code** : `lib/ordreCatalogue.ts` — les recettes que le plan peut servir (`recipeAllowed` :
     régime coché, aliments à éviter, pas de tofu chez l'omnivore D35) passent devant, les autres
     suivent ; tri STABLE, rien n'est retiré, le compteur annonce toujours tout. Branché dans
     `recettes.tsx` pour la liste « Catalogue » (Tout, créneaux, favoris, recherche) ;
     « Réalisable » inchangée. ⚠️ « Ce qu'on mange » inclut le RÉGIME, pas seulement les aliments
     évités : un seul prédicat pour le plan, « Réalisable » et cet ordre.
     **Vérifié** : aperçu web, profil du retour, recherche « poêlée » → la poêlée de tofu au quinoa
     passe de 1ʳᵉ à 7ᵉ, derrière les six compatibles ; 541 recettes toujours affichées. Garde-fou
     `lib/__tests__/ordreCatalogue.test.ts` (5 cas, **vérifié par 4 mutations**).
     ⚠️ **Remplacé le même jour par D39** : `ordreCatalogue` n'existe plus, `partagerCatalogue` le remplace.
  3 sexies. ✅ **D39 · Les recettes hors préférences attendent derrière un bouton — 2026-09-15**
     (décision fondateur, qui PRÉCISE D38). Le constat : *« quelqu'un qui cherche plus que 7
     recettes voit celles qu'elle n'aime pas apparaître »* — rangées derrière sans marque, elles
     arrivaient juste après les compatibles dès qu'une recherche en trouvait peu (« poêlée » :
     6 compatibles, puis la poêlée de tofu au quinoa). Tranché par question : un **bouton en fin
     de liste**, préféré à un séparateur et au statu quo.
     **Code** : `lib/ordreCatalogue.ts::partagerCatalogue` (servables par `recipeAllowed` /
     autres, stable, rien retiré) et `libelleAutres` (« Voir les N recettes hors de tes
     préférences », vide quand il n'y a rien derrière). `recettes.tsx` : le Catalogue ne montre
     que les compatibles ; le bouton n'arrive qu'après le dernier « Voir + » des paliers et ajoute
     les autres en fin de liste ; il se referme quand la recherche ou le filtre change ; le
     compteur annonce aussi ce qui est derrière le bouton (un chiffre ne change pas quand on
     l'ouvre) ; une liste vide avec recettes derrière dit « Aucune recette dans tes préférences
     pour « … » », pas « aucune recette ». « Réalisable » inchangée.
     **Vérifié** (aperçu web, profil du retour) : « poêlée » → 10 annoncées, 6 affichées, bouton
     pour 4 qui les ajoute ; « tofu » → message + bouton pour 38 ; recherche sans résultat → 0,
     pas de bouton. Garde-fou `ordreCatalogue.test.ts` (8 cas, **vérifié par 5 mutations**).
  3 septies. ✅ **D40 · Les trois défauts ouverts, corrigés — 2026-09-17** (« corrige les 3 »).
     Chacun avait été RE-MESURÉ dans le code avant d'être touché, pas lu dans une fiche.
     **1. Pluriel et article des aliments évités** (`lib/avoidance.ts`) : « tofu » écartait
     39 recettes, « tofus » et « le tofu » **zéro**. On retire l'article puis le « s » final ;
     ⚠️ la forme DEVINÉE doit correspondre à un **mot entier**, sinon « courges » retirerait
     tous les plats à la **courgette** — le piège `bœuf` ⊃ `œuf`, à l'autre bout du mot.
     Garde-fou `plurielArticle.test.ts`.
     **2. Renouvellement d'une semaine à l'autre** (`lib/semainePlan.ts`, pur) : RIEN ne
     regardait `week_start_date` ; `resetTracking` ne périme que le suivi du JOUR. Dix jours
     sans ouvrir Kyroz = les mêmes menus. Passé la durée du plan (bornée à 7), l'écran
     régénère avec un **nouveau tirage** (le même resservirait les mêmes plats), le suivi du
     jour conservé. Une date illisible ou dans le futur ne jette JAMAIS la semaine.
     Vérifié à l'aperçu : plan daté de 8 jours → régénéré au chargement, menus différents.
     **3. Plafond des 7 pièces végétales de D32** (`lib/repasHumain.ts`) : jamais inscrites
     dans `PROTEINES_VEGETALES_RARES`, donc 5 repas par semaine au lieu de 2.
     🔴 **CE QUE LA MESURE A CHANGÉ À L'ANNONCE** : j'avais dit « un pescétarien peut recevoir
     du jambon végétal 5 fois » — mesuré, son pire cas était 3, avant comme après. Le gain
     réel est ailleurs : semaines au-dessus du plafond, « Végétal » coché **10 → 4** (2ᵉ
     gabarit 9 → 3), pescétarien 4 → 3 (2ᵉ gabarit 4 → 1), omnivore 1 → 0, halal 0 → 0,
     aucun drapeau ajouté. Le végétarien reste à 10/12 : décision de D30 (pas de plafond chez
     lui), pas un manquement.
     ⚠️ **Le plafond est la DERNIÈRE couche d'exclusion, donc la PREMIÈRE relâchée** quand le
     vivier se vide : vérifier la règle sans vérifier le SERVI aurait déclaré le correctif
     tenu à moitié. D'où un cliquet posé sur le servi (`plafondVegetal.test.ts`), plus un test
     d'exhaustivité qui rougira au prochain aliment végétal ajouté sans plafond — c'est
     exactement ce qui a laissé le trou ouvert une semaine.
     **`ENGINE_VERSION` 57 → 58** (la sélection change). 3 fichiers de test neufs, **6
     mutations** — dont **deux sont d'abord restées VERTES** et ont révélé deux tests creux :
     un cas « pois » protégé par la LONGUEUR et non par la règle qu'il prétendait mesurer, et
     une mutation sans effet mesurable. *Une mutation qui ne rougit pas accuse le test, pas le
     code.*
  3 octies. ✅ **D42 · Les MOTS des aliments évités, et la date du plan — 2026-09-18**
     (quatre demandes du fondateur, chacune mesurée avant/après sur 38 mots).
     | mot écrit | avant → après |
     |---|---|
     | « courge » | 63 → **31** — les 32 plats à la COURGETTE ne sont plus emportés |
     | « pommes de terre » | 0 → **23** — le catalogue écrit « Pomme de terre » |
     | « patate » · « patates » | 26 → **49** — la pomme de terre s'ajoute à la patate douce |
     | « pomme » | 43 → **20** — le fruit seul |
     | « tomate » | 110 → **110** — la tomate concassée EST une tomate |
     **Code** : la fin du mot est ancrée dans `correspondAuMot`, **le pluriel excepté** —
     sans cette exception, « lentille » cesserait d'attraper « lentilles corail », soit un
     faux positif corrigé au prix de faux NÉGATIFS ; `singulier` déplie chaque mot (le
     pluriel de « pommes de terre » est sur le PREMIER) ; famille `patate` → `pomme_de_terre` ;
     table `PAS_LE_MEME_ALIMENT` (clé = `ref`) pour la pomme.
     🔴 **AUCUNE RÈGLE SYNTAXIQUE NE POUVAIT TRANCHER LA POMME.** Le catalogue n'a que DEUX
     noms d'un seul mot qui commencent un nom composé, et ils s'opposent : une tomate
     concassée EST une tomate, une pomme de terre n'est PAS une pomme. D'où une table
     NOMMÉE, et une sonde qui vérifie la tomate — sinon « généraliser » la règle de la pomme
     casserait la tomate en silence.
     ⚠️ **EFFET DE BORD ATTRAPÉ PAR LA SUITE** : les SUGGESTIONS de frappe (D37) partageaient
     la fonction de correspondance — taper « tof » a cessé de proposer « Tofu ferme ». Une
     frappe en cours est un PRÉFIXE, un filtre est un mot FINI : deux règles, `debutDeMot`
     et `correspondAuMot`.
     🔴 **ET LA DATE DU PLAN ÉTAIT EN UTC** (`week_start_date`), trouvée en testant D41 à
     **00 h 04 heure de Paris** : le plan qui venait de naître se datait de la VEILLE, alors
     que tout le reste de l'app raisonne en local. La liste de courses partait donc d'un
     jour trop tôt et le renouvellement arrivait un jour trop tard, pour quiconque génère
     son plan après minuit. `localStamp` désormais. Garde-fou `dateLocalePlan.test.ts`.
     Garde-fous : `plurielArticle.test.ts` étendu, **7 mutations**.
  3 nonies. ✅ **D43 · Trois familles d'aliments évités — 2026-09-18** (décision fondateur,
     après un AUDIT du champ demandé par lui : *« vérifie s'il y a d'autres choses à
     rectifier comme pomme de terre et courge »*).
     🔴 **CE QUE L'AUDIT N'A PAS TROUVÉ compte autant que ce qu'il a trouvé** : sur les 126
     ingrédients servis, **aucun autre faux positif** (les seuls cas restants sont vrais —
     « brocoli », « poivron » et « carotte » attrapent le « Mélange wok », qui en contient),
     **aucun faux négatif** (chaque aliment est écarté par son propre nom), et les familles
     existantes sont **complètes** (le lait de coco n'a pas de lactose, le seitan n'est pas
     du soja, les nouilles de riz n'ont pas de gluten — trois « manques » qui n'en sont pas).
     ⚠️ **La sonde a été éprouvée** : rejouée sur le commit d'avant, elle voit « Pomme →
     Pomme de terre ». Mais elle part des NOMS d'ingrédients, donc elle n'aurait jamais vu
     « courge » — ce mot n'est pas un ingrédient du catalogue (seulement « Graines de
     courge »). Les mots que les gens INVENTENT se testent par liste, pas par le catalogue :
     65 mots courants passés au crible, zéro échec silencieux.
     **Ce qui manquait était d'une autre nature** — des mots de FAMILLE qui ne désignaient
     rien : `charcuterie` 0 → **17** recettes, `viande rouge` 0 → **24**, `legumineuse(s)`
     0 → **96** (singulier, pluriel et article).
     ⚠️ **Les EXCLUSIONS sont la partie qui se garde par mutation** : le haricot VERT n'est
     pas une légumineuse (gousse mangée en légume) ; le soja TRANSFORMÉ non plus (il a sa
     famille, et l'y mettre ferait de ce mot un videur de catalogue végétal) ; « charcuterie »
     ne touche pas le jambon VÉGÉTAL. Garde-fou `famillesAliments.test.ts` (7 cas, **5
     mutations**).
  3 decies. ✅ **La page « Méthodologie & sources » remise au code — 2026-09-19** (question
     du fondateur : *« tous les changements du moteur ont été mis à jour dans la méthodologie
     et source ? »*). Réponse mesurée : **non, un seul manquait — et aucun CHIFFRE n'était
     faux.** La page lit ses nombres dans le moteur, donc elle suit les constantes ; elle ne
     suit pas les RÈGLES.
     🔴 **La détente protéique végane (−10 %, #267) était servie depuis le 2026-09-11 sans une
     ligne sur la page.** Ajoutée dans les macros ET dans « les choix qui s'écartent le plus
     de la littérature » (celle-ci plaiderait plutôt pour davantage). La phrase « même
     abaissée, elle ne descend pas sous 1,6 g/kg de masse maigre » est COMPTÉE : balayage de
     la valeur servie (287 640 profils à l'écriture, minimum 1,627) — le test des six
     gabarits ne voyait pas un Maintien baissé à 1,7 g/kg, le balayage si.
     🔴 **Plus ancien, trouvé en relisant le code** : l'escalade du plancher et la pause à la
     maintenance s'adressaient à « tout le monde », alors que le moteur les rend EXCLUSIVES
     (`dietBreakApplies`) — la remontée ne vaut que pour la femme, la pause pour l'homme et
     la femme au-delà de 40 % de MG. « Ce compteur est indépendant du précédent » faisait
     lire deux protections empilées. Le test lie désormais chaque « à qui » à son prédicat.
     ✅ **Vérifié et laissé tel quel** : le métabolisme de base reste un plancher QUOTIDIEN
     (0 jour sous le BMR sur 2 520, objectif daté serré compris — témoin : une cible de profil
     à 2 382 kcal servie à 2 443 = BMR) ; la règle Katch « mesuré » ; les DOI ; la phrase
     sur les aliments saisis à la main (vraie pour les moyennes d'étiquettes de D32).
     ⚠️ **Hors périmètre, à dessein** : les règles de CHOIX des recettes (D29 → D43, parts
     par régime) — la page dit comment Kyroz calcule, pas ce qu'il choisit.
     Au passage : « y passe , et » (reste du retrait des tirets cadratins) et « et al.. »
     sur trois références (`avecPoint`). Bloc du dump `06-textes-dump.md` régénéré depuis
     `methodologie()`, numéros de ligne compris (périmés depuis août). **5 mutations, 5
     rouges.** ⏳ **Pas encore publiée en OTA.**
     ➡️ Piste NON vérifiée, notée au passage : avec un objectif daté serré et une forte MG,
     `profile.target_kcal` (2 382) passe sous ce que le plan sert chaque jour (2 443, le
     BMR). Si un écran affiche cette cible comme « ta cible », il montre un chiffre non servi.
  4. ✅ **OTA** : D32 et D33 publiés dans la 35ᵉ, D27.1 et D34 dans la 36ᵉ, D35 dans la 37ᵉ, D36 dans la 38ᵉ, D37 dans la 39ᵉ, D38 dans la 40ᵉ, D39 dans la 41ᵉ (2026-09-15), D40 dans la 42ᵉ et D41 dans la 43ᵉ (2026-09-17), D42 dans la 44ᵉ et D43 dans la 45ᵉ (2026-09-18), B12, R6 et le zéro végétal sans case dans la 46ᵉ, les parts de protéines et le porc dans la 47ᵉ (2026-09-19) — **iOS seulement** depuis la 42ᵉ (ligne « OTA publiées »).
  ...
  6. ✅ **D41 · La liste de courses part du jour où le plan a été GÉNÉRÉ — 2026-09-17**
     (décision fondateur : *« si l'user s'inscrit le jeudi, son plan génère la semaine mais la
     liste de courses se génère sur le jeudi… et le lundi, on est lundi, donc la liste se fait
     sur la semaine »*). `lib/coursesDepuis.ts` (pur) dit quels jours la liste couvre ;
     `buildShoppingList(plan, pantry, jours?)` ignore les autres ; l'écran l'ANNONCE (« À
     partir du jeudi : ton plan avait déjà commencé »), sans quoi une liste plus courte se
     lit comme une liste incomplète.
     🔴 **LA BORNE SE FIGE À LA GÉNÉRATION** (`week_start_date`), elle ne glisse pas : sinon
     le vendredi matin, les ingrédients du jeudi DISPARAISSENT avant même que les courses
     soient faites.
     🔴 **ET CE N'EST PAS LE RANG DU JOUR — ma première version était fausse.**
     `plan_weekdays` porte le dimanche en TÊTE (`0`) alors que l'écran Plan le date en FIN de
     ...
     **Mesuré à l'aperçu** : plan généré un jeudi → 43 articles + la mention ; le même plan
     daté du lundi → 57 articles, sans mention.
     *(La seconde moitié de l'ancien point 6 — « le plan ne se régénère pas au changement de
     semaine » — est corrigée le même jour, cf. D40 point 2.)*
  ...
  8. ✅ **La hausse des répétitions — TROUVÉE et traitée, 2026-09-18/19.** Chacun des 126 commits
     moteur/catalogue du 2026-08-06 au 2026-09-15 mesuré (4 s chacun) : 8,8 % → 5,4 % (#267) →
     **25,0 % à D30** (#280, collation « sur le pouce ») → 29,6 % (D33) → 22,1 % (D34). Couches de
     D30 coupées une à une : « sur le pouce » en portait la moitié, les plafonds d'ingrédients un
     quart. 🔴 **Mais l'instrument était périmé** : `mesure-variete.ts` recopiait la clé de famille
     et n'avait jamais reçu la décision du 2026-09-14 (deux yaourts de soja à fruits différents =
     « normal ») ; la moitié de la « hausse » était ce que le fondateur avait déclaré normal.
     Traitement (décisions fondateur du 2026-09-18) : ① `familyKey` voit le FRUIT sans féculent et
     le GOÛT (D28), et les trois scripts l'IMPORTENT ; ② sept recettes différenciées — col26
     (fruits rouges), col45 (dattes), col53 (mangue), pd21 (pain perdu), rep80 (nouilles au tempeh),
     rep129 (riz complet au tofu, qui échouait R8 à 7 → 10), rep209 (seitan au riz complet) ;
     ③ **règle R6** dans `check:doublons` : une recette neuve ne rejoint AUCUNE famille occupée
     (82 familles à plusieurs recettes, cliquet), et le brief publie la liste des familles prises.
     Résultat : 22,1 % → **7,1 %** ; drapeaux 36 → 26. Vérifié par 3 mutations (R6, fruit, goût).
     ⚠️ Laissé de côté (fondateur, 2026-09-19) : `rep262` curry de crevettes au quinoa plafonne en
     glucides chez les gros gabarits en prise de masse — il a fait tomber un jour de `H3 lourd bulk`
     à −129 kcal (tolérance 90) le temps d'un tirage ; repassé au vert par effet d'enchaînement,
     donc FRAGILE.
     ⚠️ Non traité : les deux riz au lait (pd51 dattes, pd65 coco-mangue) partagent une famille ;
     aucune famille libre sans créer un autre clone, et leurs goûts diffèrent.
  9. ✅ **Qui mange de la viande sans cocher « Végétal » : ZÉRO plat principal végétal** (décision
     fondateur du 2026-09-19, « un omnivore qui ne coche rien ne doit pas avoir de végétal »). La
     règle de la case « Omnivore » (D36) s'applique à tout `mangeDeLaViande` — « Peu importe »,
     anciens comptes sans régime, halal, sans porc, sans gluten, sans lactose. Avec « Végétal »
     coché : 10 %, comme la case. Le plafond souple de D29 (3/semaine) n'existe plus. Déjeuner et
     dîner seulement : la mise en retrait du matin et de la collation, essayée, cassait 5 tests de
     calibrage et de rotation — même constat que D36. Trouvé parce qu'en « Répétitif » un profil
     sans régime recevait trois fois le même poulet végétal.
...
- **D27 · Suites de la vague B11 et de la détente protéique végane — 4 points ouverts (2026-09-13)**
  ...
  2. ✅ **Pièces végétales sans recette — B12, 2026-09-18.** Il y en avait CINQ, pas trois
     (`lardons_vegetaux` et `escalope_vegetale` aussi). Consigne fondateur : *« associe-les avec un
...
- **E73 · L'INSCRIPTION REFONDUE — 7 → 9 pages, une page par PR** — 🟡 code fait, **reste l'OTA**
...
- **E71 · Le poids servi suivait « la pesée du jour », pas la plus récente** — 🟡 code fait, **reste l'OTA**
...
- **E70 · Le jour de la pesée se CHOISIT** — 🟡 code fait, **reste : la migration + l'OTA**
...
- **E69 · La série (streak) est RETIRÉE de l'app** — 🟡 code fait, **reste à livrer**
...
- ~~**Streak**~~ — 🔴 **RETIRÉE le 2026-09-19** (décision fondateur, **E69**) : pastille, paliers, célébration, gel, synchro et table `streaks`. Ce que décrivait cette ligne (`advanceStreak`, bouclier d'un jour, `StreakCelebration`, `chainProgress`) n'existe plus dans le code ; l'historique est dans git et dans METRICS.md §2. Une série future sera repensée, pas restaurée.
...
- **Sync** (`lib/sync.ts`) : profil **colonne par colonne** (`PROFILE_COLS` + migrations idempotentes `supabase/schema.sql`) → ajouter un champ profil synchronisé = nouvelle colonne + migration. Tables profiles/favorites/pantry/weight_logs/recipe_overrides (`streaks` retirée le 2026-09-19, E69 — migration de drop à jouer après l'OTA). Edge Function `delete-account`.
- **Analytics — tunnel d'activation (`lib/analytics.ts`, ACTIF depuis le 2026-08-18 ; refondu le 2026-08-10, cf. E26)** : mesure le North Star (où les gens décrochent). `capture(event, props)` reste **NO-OP** tant que le consentement ≠ `granted` — la clé, elle, est posée (secret GitHub + variable EAS), donc c'est désormais le SEUL verrou restant. En LOCAL, `.env` reste volontairement vide (voir `.env.example`) : sans clé, consenti → `console.log('[analytics:dormant]')` en dev, pour ne pas polluer le projet de prod avec des essais. PostHog **EU** (`eu.i.posthog.com`), POST direct (pas de SDK).
  ...
  **13 events**, tous porteurs de `jour_depuis_install` (jour 0 posé à la RÉPONSE, oui ou non) : `onboarding_started/step_viewed/completed/blocked` · `first_plan_viewed` (+`duree_generation_ms`, **le moteur seul, pas la pause de 600 ms**) · `plan_opened` · `meal_cooked` · `plan_regenerated` (`origine`) · `off_plan_logged` (`recale`) · `meal_swapped` · `recipe_disliked` · `plan_generation_failed` (`etape`) · `app_error` (`ecran`, `type` — jamais le message brut). Garde-fou : `lib/__tests__/analyticsPerimetre.test.ts` (interdits du §6, liste figée à 13 — elle en comptait 15 avec `streak_milestone/frozen`, partis avec la série le 2026-09-19, E69), vérifié par 4 mutations.
