...
- **En ligne** : web sur GitHub Pages → **https://legal.kyroz.app/** (repo public `brgkevin-arch/Kyroz-app`, auto-deploy `deploy.yml` à chaque push `main`). ⚠️ **L'ancienne URL `https://brgkevin-arch.github.io/Kyroz-app/` répond 301** vers celle-ci depuis que le domaine personnalisé est posé (2026-08-18) — elle reste valide, elle ne sert plus. Le sous-domaine s'appelle `legal` pour une raison périmée (il devait porter la seule page légale) ; il sert en fait **toute l'app**. La page légale déclarée aux stores, elle, est ailleurs : `https://kyroz.app/legal.html`, dépôt `kyroz-site`. Le fondateur publie via **GitHub Desktop** (Commit→Push), pas le terminal.
...
- 🔴 **Pièges déploiement — LES DEUX ONT CHANGÉ DE SENS LE 2026-08-18, lire avant de toucher :**
  - **`baseUrl` doit rester VIDE** dans `app.json > expo.experiments` — l'inverse de la règle d'avant. Un domaine personnalisé s'applique à un SITE : la racine servie est passée de `/Kyroz-app/` à `/`, et `baseUrl="/Kyroz-app"` faisait chercher le bundle sous un sous-chemin inexistant → **404 sur le JS, splash KYROZ et spinner à l'infini**. Le site public est resté ainsi sans que rien ne rougisse : la CI déploie un artefact, elle ne visite pas la page. ⚠️ **Couplage inséparable** : si le domaine est un jour retiré, il faut remettre `baseUrl="/Kyroz-app"` ET corriger `lib/__tests__/deploiementWeb.test.ts` dans le même commit.
