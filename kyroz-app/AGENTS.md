...
| `ENGINE_VERSION` | **57** (relu le 2026-09-15 dans `lib/planEngine.ts`, dont le changelog fait foi : v57 = ni soja texturé ni tofu chez un omnivore, D35 ; v56 = soja texturé remplacé par les protéines végétales, D34 ; invalide les plans en cache). *Historique de la case, qui annonçait encore la v47 :* créneaux de repas LIBRES : l'ordre canonique de la journée devient CHRONOLOGIQUE (la collation de 16 h passe avant le dîner, elle était servie en dernier), donc le report de budget de repas en repas change d'ordre *(46 = le budget du jour suit la dépense RÉELLE du jour, `lib/dailyBudget.ts`)*. ⚠️ Cette ligne est restée à **45** pendant une journée entière après le bump : celui qui incrémente la constante est celui qui doit toucher cette case | `lib/planEngine.ts` |
...
- ⏭️ **RESTE À FAIRE — catalogue et moteur, relevé le 2026-09-14** (à lire avant D32 → D29)
  ...
  3 quater. ✅ **D37 · « Aliments à éviter » propose les sortes du catalogue — 2026-09-15**
     ...
     **Code** : `lib/avoidance.ts::suggestionsAliments` — noms tirés des RECETTES (jamais un mot
     ...
     🟡 **Toujours non traité** : « tofus » (pluriel) et « le tofu » (article) n'écartent rien.
  ...
  4. ✅ **OTA** : D32 et D33 publiés dans la 35ᵉ, D27.1 et D34 dans la 36ᵉ, D35 dans la 37ᵉ, D36 dans la 38ᵉ, D37 dans la 39ᵉ, D38 dans la 40ᵉ, D39 dans la 41ᵉ, le 2026-09-15 (ligne « OTA publiées »).
