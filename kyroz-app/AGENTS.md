...
- **E69 · La série (streak) est RETIRÉE de l'app** — 🟡 code fait, **reste à livrer**
  ...
  **Ce qui part** : la pastille « N j de série » des en-têtes Plan et Profil, la
  ...
  2. **Migration** `supabase/migrations/2026-09-19_drop_streaks.sql`, **APRÈS l'OTA**, à
     la main dans le SQL Editor. Irréversible : efface les compteurs de tous les comptes,
     et réécrit d'abord le trigger d'inscription (sans ça, plus aucune inscription ne
     passe). Puis `npm run check:migrations:prod`.
  3. **Page légale publique** (`kyroz.app/legal.html`, dépôt `kyroz-site`) : la
     régénérer avec `KYROZ_SITE=<clone> npm run gen:legal` le jour de l'OTA.
