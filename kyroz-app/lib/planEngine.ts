...
// Pénalité de score d'une recette à protéine 100 % végétale chez un omnivore qui n'a
// pas coché « Végétal » (cf. `vegetalEnRetraitIds`). Le score est l'écart calorique
// relatif : 0,05 vaut 5 % d'écart. Assez pour sortir la recette de la bande de
// départage (0,01–0,036) face à une recette animale juste ; jamais assez pour faire
// servir un repas à DRAPEAU (≥ 1 point) à sa place — la précision reste devant le goût.
const VEGETAL_EN_RETRAIT_W = 0.05;

// Pénalité de score d'une recette HORS du goût déclaré (sucré/salé, `lib/gout.ts`) sur un
// créneau où la personne en a déclaré un. Même logique que le végétal en retrait : assez
// pour sortir la recette de la bande de départage, jamais assez pour servir un repas à
// DRAPEAU (≥ 1 point) à sa place. Au-delà du score, `goutAImposer` FORCE le goût quand
// la part garantie (5 sur 7) est en jeu. Valeur calibrée à la mesure : AGENTS.md D28.
const GOUT_HORS_W = 0.15;
 ...
 */
/** Refs jamais servies à un profil qui mange de la viande (décision fondateur du 2026-09-15, D35). */
export const SOJA_TOFU_REFS: readonly string[] = ['soja_texture', 'tofu_ferme', 'tofu_fume', 'tofu_soyeux'];

/** Mange de la viande : ni végétarien, ni vegan, ni pescétarien (halal, sans porc, sans gluten… en sont). */
export function mangeDeLaViande(profile: UserProfile): boolean {
  const r = profile.dietary_restrictions ?? [];
  return !r.includes('vegan') && !r.includes('vegetarian') && !r.includes('pescatarian');
}

export function recipeAllowed(recipe: Recipe, profile: UserProfile): boolean {
  const text = ingredientText(recipe);

  // 🔴 NI SOJA TEXTURÉ NI TOFU CHEZ UN OMNIVORE (décision fondateur du 2026-09-15, D35 :
  // « je ne veux pas de PST ou TOFU chez un omnivore »). MUR DUR, comme le régime, et non
  // mise en retrait : la v50 et D29 limitaient déjà le végétal chez l'omnivore, et il en
  // recevait encore 2,5 % de ses repas, 8 % avec « Végétal » coché (mesuré le même jour).
  // Vaut AUSSI quand « Végétal » est coché : les protéines végétales de la liste (poulet
  // végétal, jambon, haché…) et le tempeh restent servis. Le pescétarien n'est pas visé.
  // Les refs d'abord ; le nom rattrape une recette personnalisée qui n'en porte pas.
  if (mangeDeLaViande(profile) && recipe.ingredients.some((i) =>
    (i.ref !== undefined && SOJA_TOFU_REFS.includes(i.ref)) || /\btofu\b|soja textur|\(pst\)/.test(i.name.toLowerCase()))) {
    return false;
  }
...
// Mots-clés par source de protéine préférée — REPLI seulement, pour une recette dont
// aucun ingrédient de rôle `protein` ne porte de `ref` (recette perso). Le catalogue,
// lui, se lit par `PROTEIN_REFS` : sur tout le texte des ingrédients, « steak » cochait
// « bœuf » sur un steak de soja et « haricot » cochait « végétal » sur des haricots verts.
...
/**
 * Refs de rôle `protein` du catalogue, rangées par clé de préférence (mêmes clés que
 * `PROTEIN_KEYWORDS`, donc que l'écran). « Poulet » couvre la VOLAILLE : la dinde n'a
 * pas d'option à elle, et quelqu'un qui coche « Poulet » ne s'attend pas à la voir
 * comptée comme une protéine qu'il n'a pas choisie.
 * ⚠️ Toute ref de protéine du catalogue doit être rangée ici OU dans
 * `PROTEIN_REFS_HORS_CHOIX`, et `preferencesServies.test.ts` le compte. Une ref oubliée
 * ne serait ni préférée ni mise en retrait, en silence.
 */
export const PROTEIN_REFS: Record<string, string[]> = {
  ...
  whey: ['whey'],
  'végétal': [
    'soja_texture', 'yaourt_soja_proteine', 'proteine_vegetale', 'tofu_ferme', 'tofu_fume', 'tofu_soyeux',
    'pois_chiches_conserve', 'edamame', 'seitan', 'tempeh', 'haricots_rouges_conserve',
    'haricots_noirs_conserve', 'haricots_blancs_conserve', 'lentilles_cuites', 'lentilles_corail', 'feves',
    'pois_casses', 'falafel', 'levure_maltee', 'steak_soja', 'emince_vegetal', 'hache_vegetal',
    'galette_vegetale', 'boulette_vegetale', 'nuggets_vegetal', 'saucisse_vegetale', 'jambon_vegetal',
    'filets_poulet_vegetal', 'aiguillettes_poulet_vegetal', 'chorizo_vegetal', 'lardons_vegetaux',
  ],
};

/** Protéines qu'aucune case de l'écran ne désigne (laitages, charcuterie) : ni préférées, ni en retrait. */
export const PROTEIN_REFS_HORS_CHOIX = [
  'skyr', 'cottage_cheese', 'fromage_blanc_0', 'yaourt_grec', 'feta', 'parmesan', 'mozzarella', 'jambon_blanc',
];

const proteinRefsOf = (r: Recipe): string[] =>
  r.ingredients.filter((i) => i.macro_role === 'protein' && i.ref).map((i) => i.ref!);

/** Ids des recettes dont la protéine matche ≥1 protéine préférée. */
function preferredRecipeIds(profile: UserProfile): Set<string> {
  const prefs = profile.preferred_proteins ?? [];
  const refs = new Set(prefs.flatMap((p) => PROTEIN_REFS[p] ?? []));
  const kws = prefs.flatMap((p) => PROTEIN_KEYWORDS[p] ?? []);
  if (kws.length === 0 && refs.size === 0) return new Set();
  const ids = new Set<string>();
  for (const r of getEffectiveRecipes()) {
    const pr = proteinRefsOf(r);
    const match = pr.length > 0 ? pr.some((x) => refs.has(x)) : kws.some((kw) => ingredientText(r).includes(kw));
...
/**
 * Recettes à protéine 100 % VÉGÉTALE, mises EN RETRAIT (pas exclues) chez quelqu'un qui
 * mange de la viande ou du poisson, a déclaré ses protéines, et n'a PAS coché « Végétal ».
 *
 * 🔴 LE DÉFAUT (signalé par le fondateur le 2026-09-12, mesuré le même jour) : un homme
 * de 84 kg ayant coché poulet/bœuf/poisson/œufs/whey recevait **25 %** de repas à
 * protéine 100 % végétale — ragoût de soja le soir, galette végétale le midi. La
 * préférence n'était qu'un DÉPARTAGE entre recettes à 1 % d'écart ; les vagues
 * végétales (B7, B11), écrites pour les véganes et bien calibrées en macros, passaient
 * donc devant sans que rien ne s'y oppose. Ne pas cocher une case ne disait rien.
 *
 * ⚠️ « Peu importe » (liste vide) ne met RIEN en retrait, et un profil végétarien ou
 * vegan non plus : pour eux, le végétal est le plat, pas une substitution.
 * 🔴 SAUF EN HALAL ET SANS PORC (décision fondateur du 2026-09-13, D29) : ces deux
 * profils sont traités comme un omnivore qui a coché ses protéines, même sur « Peu
 * importe ». Leur régime dit déjà ce qu'ils mangent : de la viande, sans porc.
 */
function vegetalEnRetraitIds(profile: UserProfile): Set<string> {
  const prefs = profile.preferred_proteins ?? [];
  ...
  for (const r of getEffectiveRecipes()) {
    const pr = proteinRefsOf(r);
    if (pr.length > 0 && pr.every((x) => vegetal.has(x))) ids.add(r.id);
  }
  return ids;
}
...
export interface RegleVegetal {
  /** Recettes qui comptent dans le plafond. */
  ids: Set<string>;
  /** Plafond sur la semaine (`Infinity` = aucun). */
  maxSemaine: number;
  /** Plafond sur une journée. */
  maxJour: number;
}

/**
 * COHÉRENCE D'UNE JOURNÉE : combien de plats végétaux un profil qui mange de la viande
 * ou du poisson reçoit au déjeuner et au dîner (décision fondateur du 2026-09-13, D29).
 *
 * 🔴 LE DÉFAUT, lu dans le carnet des menus le 2026-09-13 : l'omnivore qui répond « Peu
 * importe » aux protéines ouvrait sa semaine sur un lundi 100 % végétal (edamame matin,
 * midi et soir, près de 700 g), et 57 % de ses journées portaient un déjeuner ou un dîner
 * 100 % végétal. La mise en retrait de la v50 ne le touchait pas : elle ne s'arme que
 * sur des protéines cochées, et « Peu importe » est une liste vide.
 *
 *  - qui mange de la viande (case « Omnivore », « Peu importe », sans lactose, sans gluten,
 *    halal, sans porc) : AUCUN déjeuner ni dîner à protéine 100 % végétale ; avec « Végétal »
 *    coché, 10 % d'entre eux (D36, étendu à tous les mangeurs de viande le 2026-09-19 — le
 *    plafond de 3 par semaine de D29 n'existe plus) ;
 *  - pescétarien : au plus un déjeuner ou dîner sans poisson par jour. Ce repas-là est
 *    VÉGÉTARIEN, pas forcément végan : les œufs et les laitages y comptent ;
 *  - végétarien, vegan (et pescétarien avec « Végétal » coché) : aucune règle.
 *
 * ⚠️ Le petit-déjeuner et la collation ne comptent pas : un yaourt de soja à 16 h n'est
 * pas « un repas végétal » pour qui le mange.
 */
export function regleVegetal(profile: UserProfile): RegleVegetal | null {
  ...
  // 🔴 ET SANS RIEN COCHER, C'EST LA MÊME RÈGLE (décision fondateur du 2026-09-19 : « un omnivore
  // qui ne coche rien ne doit pas avoir de végétal »). Jusque-là, un profil sans régime (ancien
  // compte, case laissée vide, ou seulement halal / sans porc / sans gluten / sans lactose) gardait
  // la règle souple de D29 — 3 plats végétaux par semaine — et, en « Répétitif », pouvait recevoir
  // trois fois le même poulet végétal. Qui mange de la viande reçoit ce que reçoit la case Omnivore.
  if (mangeDeLaViande(profile)) {
  ...
  // Pescétarien (le seul cas restant : qui mange de la viande est traité plus haut, végétarien
  // et vegan n'ont aucune règle). L'ancien plafond « omnivore » de D29 (3 par semaine, 1 par
  // jour) a disparu le 2026-09-19 : plus aucun profil ne l'atteignait.
  const ids = new Set<string>();
  const poisson = new Set(PROTEIN_REFS.poisson);
  for (const r of getEffectiveRecipes()) if (!proteinRefsOf(r).some((x) => poisson.has(x))) ids.add(r.id);
  return { ids, maxSemaine: Infinity, maxJour: PESCE_SANS_POISSON_MAX_JOUR };
}
 ...
 *  - repetitive : pas de rotation ni de famille, mais préférences, besoin et fibres
 *    comme ailleurs, et au plus ceil(jours/2) services par recette (v50).
 ...
 */
/**
 * Clé de FAMILLE d'une recette : `refs protéine × refs glucide` (× fruit sans glucide).
 *
 * ⚠️ SOURCE UNIQUE : `mesure-variete.ts`, `mesure-reglages.ts` et `varieteFamille.test.ts`
 * l'IMPORTENT. Jusqu'au 2026-09-18 chacun en recopiait la formule ; le script de mesure n'a
 * donc jamais appris la décision du 2026-09-14, et il a accusé D30 d'une hausse de 5 à 25 %
 * qui était, pour moitié, ce que le fondateur venait de déclarer normal.
 *
...
export function familyKey(r: Recipe): string {
  ...
  const carb = refs('carb');
  // 🔴 SANS FÉCULENT, LE FRUIT FAIT LE SECOND AXE (décision fondateur du 2026-09-18).
  // `protéine × ∅` rangeait dans UNE famille huit collations au yaourt de soja qu'un humain
  // distingue (pudding de chia aux framboises, compote pomme-chia, smoothie à l'ananas) — le
  // fondateur l'avait tranché le 2026-09-14 (« non, c'est normal »), mais seul un test l'avait
  // appris. À l'inverse, « chaque recette sans féculent est sa propre famille » laisserait
  // passer les vrais clones (même yaourt, même banane). Même fruit = même assiette ; fruit
  // différent = autre plat. Sans fruit non plus, la clé reste `∅`.
  // Et le GOÛT (D28) sépare ce que protéine × féculent confond : un pain perdu à la banane et
  // des blancs d'œufs brouillés sur pain complet ont la même clé, pas la même assiette.
  const fruit = refs('fruit');
  return `${refs('protein')}×${carb !== '∅' ? carb : fruit !== '∅' ? `fruit:${fruit}` : '∅'}|${goutRecette(r)}`;
}

/**
 * Le REGISTRE d'une recette — à quoi ressemble l'assiette, pas ce qu'elle contient (D25).
 *
 * `familyKey` distingue poulet-riz de tofu-riz ; il ne distingue PAS un porridge d'une
 * tartine, parce que les deux peuvent porter le même couple protéine × féculent. Or c'est
 * exactement la répétition que la personne VOIT au réveil : sept petits-déjeuners
 * d'affilée en porridge, poudres et soja (signalé par le fondateur le 2026-09-07, puis
 * de nouveau le 2026-09-12 sur son propre plan).
 *
 * ⚠️ POURQUOI UNE ROTATION ET PAS UN MEILLEUR SCORE. Trois correctifs par le score ont été
 * MESURÉS inefficaces le 2026-09-07 (fiche D25) : pénaliser l'écart aux glucides fait
 * BAISSER la part de pain (12 % → 10 %), élargir la bande de variété jusqu'à 0,08 ne rend
 * que 1 à 2 points, et réécrire les recettes moins protéinées améliore leur couverture
 * sans changer leur rang. Les petits-déjeuners tiennent dans 5 % d'écart au score, le
 * panier n'en retient que 8 à 29, et le format « pain » y est structurellement 1 % plus
 * loin. Aucun réglage de score ne rattrape ça ; seule une contrainte de DIVERSITÉ le fait.
 *
 * ⚠️ Rédigé sur la branche fermée #230 (2026-09-07), jamais fusionné, et REPRIS le
 * 2026-09-13 à la demande du fondateur, par-dessus la v50.
 */
export function registreKey(r: Recipe): string {
  const refs = r.ingredients.map((i) => i.ref ?? '');
  const a = (...x: string[]) => x.some((k) => refs.includes(k));
  if (a('whey', 'proteine_vegetale', 'soja_texture')) return 'poudre';
  if (a('pain_complet', 'pain_seigle', 'pain_pita_complet', 'pain_sans_gluten', 'tortilla_complete', 'wrap_sans_gluten', 'galette_riz')) return 'pain';
  if (a('edamame', 'seitan', 'tempeh', 'tofu_ferme', 'tofu_fume', 'tofu_soyeux', 'yaourt_soja_proteine')) return 'vegetal';
  if (a('flocons_avoine', 'millet', 'polenta', 'riz_basmati', 'riz_complet', 'semoule_couscous', 'quinoa', 'sarrasin', 'chataigne')) return 'porridge';
  return 'autre';
}

/**
 * Poids de la rotation par registre, calibré au balayage (0 · 0,02 · 0,04 · 0,08) sur les
 * 12 profils × 4 semaines, le 2026-09-07 : 0,08 retenu. Plus GRAND que `FAMILY_SELECT_W_*`
 * (0,04) parce qu'il y a 5 registres pour ~120 familles : chaque registre est resservi
 * bien plus souvent, donc il faut un cran plus ferme pour qu'il sorte de la bande.
 * ⚠️ Éteint en « répétitif », comme la famille : ce réglage DEMANDE la même base.
 * Chiffres re-mesurés à la reprise : AGENTS.md D25.
 */
const REGISTRE_SELECT_W = 0.08;

/**
 * Table vide passée aux créneaux qui ne sont PAS un petit-déjeuner : la rotation par
 * registre n'y agit pas.
 *
 * 🔴 LA VERSION FERMÉE LE FAISAIT TOURNER SUR TOUS LES REPAS, et ça ne se voyait pas sur
 * son propre test (qui ne regarde que le petit-déjeuner). Mesuré à la reprise, le
 * 2026-09-13, sur le gabarit du fondateur (omnivore, « Végétal » non coché) : les repas à
 * protéine 100 % végétale passaient de 3 % à **14 %**, surtout au déjeuner et au dîner.
 * `registreKey` y range tofu, tempeh et seitan dans « vegetal » et riz ou quinoa dans
 * « porridge » : faire tourner ces registres le midi, c'est ramener un plat végétal pour
 * « varier », par-dessus la mise en retrait de la v50. Monter `VEGETAL_EN_RETRAIT_W` ne
 * réglait rien proprement (0,15 : encore 6 à 7 % ; 0,25 : 2 %, mais 3 repas hors cible).
 * Le défaut signalé est celui du RÉVEIL ; le correctif y reste.
 */
const SANS_REGISTRE = new Map<string, string>();

/**
 * La rotation par registre s'applique-t-elle à ce profil ? Non pour un profil végétarien
 * ou vegan.
 *
 * 🔴 MESURÉ le 2026-09-13 (12 gabarits × 5 régimes × 4 tirages, 6 720 repas) : repas
 * hors cible **13** sur `main`, **15** avec le seul catalogue B10, **23** avec la
 * rotation partout, **17** sans rotation pour ces deux régimes. Les 8 de trop ne sont
 * PAS au petit-déjeuner : ils tombent le midi, le soir et à la collation, sur des
 * recettes anciennes. Forcer un autre registre au réveil sur un vivier mince change ce
 * qui reste du budget du jour, et le vivier végétal n'a pas de quoi le remplir.
 * ⚠️ Le défaut que la rotation corrige (poudres et soja au réveil chez quelqu'un qui n'en
 * a rien demandé) est un défaut d'OMNIVORE : les petits-déjeuners « à la française »
 * (12 gabarits sans régime) passent de 63 à 68 % sans elle chez les végétariens.
 * ➡️ À rouvrir le jour où le vivier végétal du matin s'épaissit, en re-mesurant ce couple.
 */
export function rotationRegistreActive(profile: UserProfile): boolean {
  const regimes = profile.dietary_restrictions ?? [];
...
function selectMealAdapted(
  ...
  registreUsage: Record<string, number>,
  registres: Map<string, string>,
  variety: VarietyPreference,
  preferredIds: Set<string>,
  enRetraitIds: Set<string>,
  maxUsageRepetitif: number,
  goutMatch: Set<string> | null,
  goutImpose: boolean,
  exclusions: ((r: Recipe) => boolean)[],
    ...
    .map((r) => {
      ...
      return {
        ...
        score: fitScore(a.macros, target, a.flags, goalDir)
          + (enRetraitIds.has(r.id) ? VEGETAL_EN_RETRAIT_W : 0)
          + (goutMatch && !goutMatch.has(r.id) ? GOUT_HORS_W : 0),
  ...
  // 🔴 « RÉPÉTITIF » VEUT DIRE « SOUVENT LES MÊMES », PAS « TOUJOURS LE MÊME » (décision
  // fondateur, 2026-09-12 : « au moins deux avec la même base pour la semaine »).
  // Avant, le plan canonique répétitif sortait ICI sur `candidates[0]` — le meilleur fit
  // macro, sans préférence, sans plafond : mesuré sur H 84 kg, le même plat revenait
  // **6 jours sur 7** et 36 % des repas ignoraient les protéines déclarées. Désormais il
  // suit le chemin commun (préférences, besoin, fibres ; rotation et famille restent
  // éteintes), et une recette ne dépasse pas `maxUsageRepetitif` services par semaine —
  // la moitié des jours, arrondie au-dessus, donc toujours ≥ 2 plats par créneau.
  // ⚠️ Le plafond ne fait JAMAIS servir un repas à drapeau : s'il ne reste que des
  // candidats hors cible sous le plafond alors qu'un propre existe au-dessus, on garde
  // le propre. La précision passe devant la variété, comme partout ailleurs.
  const propre = (c: AdaptedChoice) => !c.flags.some((f) => FLAG_AUDIENCE[f] === 'user');
  let candidates = tous;
  // COHÉRENCE DE LA JOURNÉE ET LECTURE HUMAINE (D29, D30) : des COUCHES d'exclusion, dans
  // l'ordre où l'appelant les range (la plus importante d'abord). Chacune ne s'applique
  // que s'il reste un candidat PROPRE après elle — même arbitrage que le goût : une règle
  // ne fabrique jamais un repas mal calibré.
  // ⚠️ COUCHE PAR COUCHE, et pas toutes d'un bloc : sur un vivier mince (vegan sans
  // gluten), l'ensemble peut ne laisser aucun plat propre alors que les premières
  // couches en laissaient. Tout lâcher d'un coup rendrait « même plat deux fois le même
  // jour » pour ne pas tenir « ingrédient rare au plus 2 fois ».
  ...
  if (variety === 'repetitive') {
    ...
    if (sousPlafond.length > 0 && (sousPlafond.some(propre) || !candidates.some(propre))) candidates = sousPlafond;
  }
  // GOÛT GARANTI (D28) : quand la part promise (5 sur 7) ne tiendrait plus sans ce service,
  // on ne garde que les recettes du goût déclaré — à condition qu'il en reste une PROPRE.
  // Sinon on laisse le panier tel quel : un repas juste d'un autre goût vaut mieux qu'un
  // repas à drapeau du bon goût, et c'est ce que « jamais un repas mal calibré » veut dire.
  if (goutMatch && goutImpose) {
    const dansLeGout = candidates.filter((c) => goutMatch.has(c.recipe.id));
    if (dansLeGout.some(propre)) candidates = dansLeGout;
  }
  ...
  const regW = famActive ? REGISTRE_SELECT_W : 0;
  // 🔴 Une recette EN RETRAIT (végétal chez l'omnivore qui ne l'a pas coché) compte comme
  // le registre le PLUS servi, jamais comme un registre à découvrir. Sans ça la rotation
  // la présentait comme la nouveauté de la semaine et défaisait la v50 : mesuré le
  // 2026-09-13 sur le gabarit du fondateur, 0 → 8–11 % de petits-déjeuners végétaux, dont
  // le bol edamame-millet qu'il avait signalé. Hors petit-déjeuner la table est vide
  // (`SANS_REGISTRE`), donc ce plancher n'y ajoute rien.
  const regUseMax = registres.size > 0 ? Math.max(0, ...Object.values(registreUsage)) : 0;
  const regUse = (c: AdaptedChoice) => {
    if (registres.size === 0) return 0;
    if (enRetraitIds.has(c.recipe.id)) return regUseMax;
    return registreUsage[registres.get(c.recipe.id) ?? ''] ?? 0;
  };
  const effOf = (c: AdaptedChoice) =>
    c.score - fiberW * c.fiber + step * (usage[c.recipe.id] ?? 0) + famW * famUse(c) + regW * regUse(c);
...
export function profileSignature(p: UserProfile): string {
  ...
  return JSON.stringify({
    ...
    // Goût déclaré (D28). Lu par `goutLu` : une valeur inconnue signe comme « peu importe ».
    gp: goutLu(p.gout_petit_dej) ?? null, gc: goutLu(p.gout_collation) ?? null,
...
export function buildLocalPlan(profile: UserProfile, seed: number = 0): MealPlan {
  ...
  // Recettes correspondant aux protéines préférées (départage à macro égale), et
  // recettes 100 % végétales en retrait chez l'omnivore qui n'a pas coché « Végétal ».
  const preferredIds = preferredRecipeIds(profile);
  const enRetraitIds = vegetalEnRetraitIds(profile);
  // Plafond de plats végétaux au déjeuner et au dîner (D29), compté sur la semaine et sur le jour.
  const regle = regleVegetal(profile);
  let vegetalSemaine = 0;
  // LECTURE HUMAINE (D30) : repas de la semaine où chaque ingrédient de base (protéine ou
  // féculent) a déjà été servi, contre son plafond (`plafondIngredient` : courant 5, rare 2).
  const ingredientSemaine: Record<string, number> = {};
  const plafondDepasse = (r: Recipe) =>
    ingredientsDeBase(r).some((ref) => (ingredientSemaine[ref] ?? 0) >= plafondIngredient(ref, profile));
  // MÊME PLAT (« Équilibré ») : le dîner des jours 1, 3 et 5 revient au déjeuner du lendemain,
  // à condition que le lendemain soit le jour CALENDAIRE suivant (un plan lundi-mercredi-
  // vendredi ne sert pas le dîner de lundi mercredi midi).
  const JOURS_MEME_PLAT = new Set([1, 3, 5]);
  const wd = profile.plan_weekdays ?? [];
  const lendemainCalendaire = (d: number) =>
    wd.length >= d + 1 ? (wd[d] - wd[d - 1] + 7) % 7 === 1 : true;
  // Plafond de services d'une même recette en « répétitif » : la moitié des jours, arrondie au-dessus.
  const maxUsageRepetitif = Math.max(1, Math.ceil(days / 2));

  // GOÛT DÉCLARÉ (sucré/salé, D28), par VIVIER : un créneau créé en vivier collation
  // suit la réponse « collation », exactement comme la collation intégrée.
  // `goutIds` = les recettes du bon goût dans les pools de ce vivier ; `goutServices` =
  // combien de fois ce vivier sera servi dans la semaine (repas « je gère » exclus).
  const goutParVivier: Record<string, GoutPreference | undefined> = {
    breakfast: goutLu(profile.gout_petit_dej), snack: goutLu(profile.gout_collation),
  };
  const goutIds: Record<string, Set<string>> = {};
  // Forçage de la part garantie : pas pour un profil vegan (cf. `goutGarantiPour`).
  const goutGaranti = goutGarantiPour(profile.dietary_restrictions);
  const goutServices: Record<string, number> = {};
  const goutServis: Record<string, number> = {};
  const goutPasses: Record<string, number> = {};
  for (const slot of daySlots) {
    const g = goutParVivier[slot.pool];
    if (!g || fixedMeals[slot.id]) continue;
    const ids = (goutIds[slot.pool] ??= new Set<string>());
    for (const r of pools[slot.id] ?? []) if (goutRecette(r) === g) ids.add(r.id);
    goutServices[slot.pool] = (goutServices[slot.pool] ?? 0) + days;
  }
  ...
  // Idem un cran plus haut : le REGISTRE (pain, porridge, végétal, poudre), cf. `registreKey`.
  // ⚠️ PETIT-DÉJEUNER SEULEMENT, cf. `SANS_REGISTRE` : la table ne connaît que les recettes
  // des créneaux de vivier `breakfast`, et seuls ces créneaux la reçoivent et la font avancer.
  const registreUsage: Record<string, number> = {};
  const registres = new Map<string, string>();
  const rotationRegistre = rotationRegistreActive(profile);
  for (const slot of daySlots) {
    if (slot.pool !== 'breakfast' || !rotationRegistre) continue;
    for (const r of pools[slot.id] ?? []) if (!registres.has(r.id)) registres.set(r.id, registreKey(r));
  }
  ...
  for (let d = 1; d <= days; d++) {
    ...
    const dayMeals: Meal[] = [];
    // D29 : recettes déjà servies aujourd'hui, et plats végétaux servis au déjeuner et au dîner.
    const servisDuJour = new Set<string>();
    let vegetalJour = 0;
    // D30 : féculents déjà servis aujourd'hui (le même au plus une fois par jour).
    const feculentsDuJour = new Set<string>();
    // Le plafond de la semaine s'ÉTALE : au jour d, au plus ceil(max × d / jours) plats
    // végétaux servis. Sans ça les trois tombent lundi, mardi et mercredi, c'est-à-dire
    // sur la première impression de qui vient de s'inscrire.
    const vegetalPermisAuJour = regle ? Math.min(regle.maxSemaine, Math.ceil((regle.maxSemaine * d) / days)) : Infinity;
    for (const slot of daySlots) {
      const mealType = slot.id;
      ...
      const principal = slot.pool === 'meal';
      const plafondAtteint = !!regle && principal
        && (vegetalJour >= regle.maxJour || vegetalSemaine >= vegetalPermisAuJour);
      // Couches d'exclusion, LA PLUS IMPORTANTE D'ABORD (cf. `selectMealAdapted`) :
      // même recette le même jour · plafond végétal (D29) · plat du midi au petit-déjeuner ·
      // collation qui n'est pas sur le pouce · même féculent le même jour · plafond
      // d'ingrédient de la semaine (D30).
      const exclusions: ((r: Recipe) => boolean)[] = [
        (r) => servisDuJour.has(r.id),
        (r) => plafondAtteint && regle!.ids.has(r.id),
        ...(slot.pool === 'breakfast' ? [platDuMidi] : []),
        ...(slot.pool === 'snack' ? [(r: Recipe) => !surLePouce(r)] : []),
        // ⚠️ PAS EN « RÉPÉTITIF » : ce réglage DEMANDE de remanger les mêmes plats, et son
        // plafond par recette (`ceil(jours/2)`, D26) gouverne déjà la répétition. Mesuré le
        // 2026-09-14 sur un profil sans sport : ces deux couches portaient le créneau le plus
        // répétitif de 2 à 5 plats distincts, et « souvent les mêmes » ne voulait plus rien dire.
        ...(variety === 'repetitive' ? [] : [
          (r: Recipe) => feculentsDe(r).some((f) => feculentsDuJour.has(f)),
          plafondDepasse,
        ]),
      ];
      ...
      const goutTotal = goutServices[slot.pool] ?? 0;
      const goutQuota = quotaGout(goutTotal);
      const goutDejaServis = goutServis[slot.pool] ?? 0;
      // 🔴 PROMESSE TENUE → LE GOÛT CESSE DE PESER. « Majorité garantie » veut dire 70 %,
      // pas le maximum possible. Mesuré le 2026-09-13 en le laissant peser toute la
      // semaine : « salé » servait 92 % de petits-déjeuners salés, et les repas mal
      // calibrés doublaient chez les véganes (vegan 4 → 14, vegan sans gluten 7 → 19),
      // dont le vivier salé est trois fois plus mince que le sucré.
      const goutMatch = goutIds[slot.pool] && goutDejaServis < goutQuota ? goutIds[slot.pool] : null;
      const goutImpose = goutGaranti && !!goutMatch
        && goutAImposer(goutQuota, goutDejaServis, goutTotal - (goutPasses[slot.pool] ?? 0));

      // MÊME PLAT (« Équilibré », D30) : au premier créneau principal du lendemain d'un jour
      // de même plat, on ressert le dîner de la veille, ADAPTÉ à la cible de CE déjeuner —
      // ses quantités sont les siennes, jamais une part prélevée sur le dîner — à condition
      // qu'aucune couche ne l'exclue et que l'adaptation reste propre. Sinon, sélection
      // ordinaire, et le dîner de la veille ne porte aucune mention.
      const premierPrincipalDuJour = principal && !dayMeals.some((m) => !m.fixed && slotOrFallback(slots, m.meal_type).pool === 'meal');
      const veille = variety === 'balanced' && premierPrincipalDuJour && d > 1 && JOURS_MEME_PLAT.has(d - 1) && lendemainCalendaire(d - 1)
        ? [...meals].reverse().find((m) => m.day === d - 1 && !m.fixed && slotOrFallback(slots, m.meal_type).pool === 'meal')
        : undefined;
      let memePlat: AdaptedChoice | null = null;
      if (veille && !exclusions.some((ex) => ex(veille.recipe))) {
        const a = adaptRecipe(veille.recipe, target);
        if (!a.flags.some((f) => FLAG_AUDIENCE[f] === 'user')) {
          memePlat = {
            recipe: veille.recipe, ingredients: a.ingredients, macros: a.macros, gap: a.gap, flags: a.flags,
            score: 0, fiber: 0, preferred: false, need: 0,
          };
      ...
      for (const ref of ingredientsDeBase(choice.recipe)) ingredientSemaine[ref] = (ingredientSemaine[ref] ?? 0) + 1;
      if (regle && principal && regle.ids.has(choice.recipe.id)) { vegetalJour++; vegetalSemaine++; }
      ...
      if (goutMatch) {
        goutPasses[slot.pool] = (goutPasses[slot.pool] ?? 0) + 1;
        if (goutMatch.has(choice.recipe.id)) goutServis[slot.pool] = (goutServis[slot.pool] ?? 0) + 1;
      }
      const reg = slot.pool === 'breakfast' && rotationRegistre ? registres.get(choice.recipe.id) : undefined;
      if (reg) registreUsage[reg] = (registreUsage[reg] ?? 0) + 1;
      ...
      dayMeals.push({
        ...
        rest_day: isRest || undefined,
        same_dish_yesterday: memePlat ? true : undefined,
      });
    }
...
export function swapMeal(profile: UserProfile, plan: MealPlan, meal: Meal, favoriteIds?: Iterable<string>): MealPlan {
  ...
  const newMeal: Meal = {
    ...
    same_dish_tomorrow: undefined,
    same_dish_yesterday: undefined,
  };
  // MÊME PLAT (D30) : remplacer l'un des deux repas d'une paire casse la paire. Sans ça, le
  // dîner annoncerait « même plat demain midi » pour un déjeuner qui n'est plus le sien.
  const meals = accorderMemePlat(plan.meals.map((m) => (m.id === meal.id ? newMeal : m)));
  return { ...plan, meals, total_macros_per_day: computeDailyTotals(meals, plan.days, plan.day_extras) };
}

/**
 * MÊME PLAT (D30) : une mention ne vaut que si sa paire existe ENCORE — dîner du jour d
 * marqué « demain », déjeuner du jour d+1 marqué « hier », même recette. Sinon les deux
 * tombent. On n'en crée jamais : seule la génération décide d'une paire.
 *
 * Appelé partout où un repas peut changer de recette APRÈS la génération : « Remplacer ce
 * repas » (`swapMeal`) et la régénération qui reporte un repas mangé ENTIER, mentions
 * comprises (`carryTracking`). Rend le même tableau quand rien ne tombe.
 */
export function accorderMemePlat(meals: Meal[]): Meal[] {
  const cle = (m: Meal) => `${m.day}|${m.recipe.id}`;
  const attenduDemain = new Set(meals.filter((m) => m.same_dish_tomorrow).map((m) => `${m.day + 1}|${m.recipe.id}`));
  const attenduHier = new Set(meals.filter((m) => m.same_dish_yesterday).map((m) => `${m.day - 1}|${m.recipe.id}`));
  let change = false;
  const out = meals.map((m) => {
    const perdDemain = !!m.same_dish_tomorrow && !attenduHier.has(cle(m));
    const perdHier = !!m.same_dish_yesterday && !attenduDemain.has(cle(m));
    if (!perdDemain && !perdHier) return m;
    change = true;
    return {
      ...m,
      same_dish_tomorrow: perdDemain ? undefined : m.same_dish_tomorrow,
      same_dish_yesterday: perdHier ? undefined : m.same_dish_yesterday,
    };
  });
  return change ? out : meals;
}
...
export function carryTracking(profile: UserProfile, ancien: MealPlan | null | undefined, nouveau: MealPlan): MealPlan {
  ...
  // Un repas mangé revient ENTIER, mention « même plat » comprise (D30) : si le plan neuf ne
  // ressert plus ce plat de l'autre côté de la paire, `accorderMemePlat` fait tomber les deux.
  const meals = accorderMemePlat(nouveau.meals.map((m) => {
