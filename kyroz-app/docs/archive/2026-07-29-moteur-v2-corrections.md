# ARCHIVÉ — DOCUMENT D'HISTOIRE · Corrections du moteur de calcul (v2)

> **Archivé le 2026-07-30.** Ce document le disait déjà de lui-même : il est devenu un
> document d'histoire (constat du 2026-07-29, 18 agents, 85 points mesurés en exécutant
> le moteur — la spec s'est révélée fausse plusieurs fois). Les corrections P0 et P1
> sont **livrées** ; P2 n'est pas arbitré par ce fichier.
>
> ⚠️ Une de ses affirmations est **contredite par le code**, mesuré le 2026-07-30 :
> « le moteur reste 100 % déterministe, mêmes entrées = mêmes sorties » est vrai pour
> `buildLocalPlan` (variation pilotée par `seed`) mais **faux pour `swapMeal`**, qui
> tire avec `Math.random()` (`lib/planEngine.ts:787`), et le plan lit l'horloge pour son
