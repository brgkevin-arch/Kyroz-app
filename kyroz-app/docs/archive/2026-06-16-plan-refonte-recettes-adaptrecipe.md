# ARCHIVÉ — LIVRÉ · Refonte recettes (100) + adaptRecipe (scaling par ingrédient)

> # 🛑 NE PAS EXÉCUTER CE PLAN. IL EST DÉJÀ FAIT.
>
> **Archivé le 2026-07-30.** Ce plan porte **79 cases `- [ ]` non cochées sur 79**, et
> c'est un artefact : personne n'a coché en avançant. Le travail est **livré et en
> production** — vérifié le 2026-07-30 :
>
> - `lib/adaptRecipe.ts` existe (scaling par ingrédient, drapeaux, écart)
> - le catalogue compte **314 recettes** (le plan en visait 100)
> - `lib/__tests__/adaptRecipe.test.ts` est vert, dans une suite de 563 tests
>
> Un agent (ou un humain) qui ouvre ce fichier et lit ses cases à cocher conclura
> qu'il reste 79 tâches. **C'est faux.** Ce document ne vaut plus que comme trace de
> la démarche de juin 2026.
>
> État courant du build → `AGENTS.md`. Spec stable → `CLAUDE.md`.

<details>
<summary>Contenu d'origine (historique)</summary>
