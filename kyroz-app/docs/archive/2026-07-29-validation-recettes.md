# ARCHIVÉ — SANS OBJET · Dossier de validation diététicienne

> # 🛑 CE DOSSIER N'A PLUS D'OBJET.
>
> **Archivé le 2026-07-30, par décision du fondateur.** Son but était de faire valider
> les recettes par une diététicienne-nutritionniste diplômée avant mise en production.
> **Cette validation est ÉCARTÉE** (décision du 2026-07-29, cf. `CLAUDE.md` §6). Ce
> n'est pas un retard à rattraper, c'est un choix : **ne plus la remonter** comme
> prérequis, ni dans un bilan, ni dans une recommandation de chantier.
>
> **Ce fichier est FIGÉ.** Il n'est plus régénérable : le script qui le produisait
> (`scripts/gen-validation-recettes.ts`) et son entrée npm (`gen:validation`) ont été
> supprimés le 2026-07-30. Les chiffres ci-dessous sont donc une **photo au 2026-07-29**
> (314 recettes) et vont dériver du catalogue réel.
>
> **Pour mesurer le catalogue aujourd'hui**, les outils vivants sont :
> `npm run mesure:couverture` (vérité terrain sur 12 profils, règle R8) et
> `npm run check:doublons`. Ce sont eux qui arbitrent le chantier « réparer le
> catalogue » — pas ce fichier.
>
> Ce qui reste vrai : `validated_by_dietitian` est `false` **en dur**
> (`lib/recipeMap.ts`), donc aucun écran ne doit prétendre le contraire, et le
> disclaimer de `CLAUDE.md` §6 en devient d'autant plus obligatoire.
