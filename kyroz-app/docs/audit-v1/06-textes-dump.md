...
> 37 textes, contre 72 fragments.

| # | ligne | Rôle | Texte | Flag |
|---|---|---|---|---|
| 1 | `lib/methodologie.ts:60` | titre | Ce que Kyroz calcule, et ce qu'il n'est pas |  |
| 2 | `lib/methodologie.ts:62` | corps | Kyroz estime une dépense énergétique quotidienne à partir de ce que tu déclares, puis construit des repas qui s'en approchent. C'est un outil de bien-être alimentaire pour adultes en bonne santé. |  |
...
| 8 | `lib/methodologie.ts:79` | corps | Par défaut, c'est Mifflin-St Jeor qui sert : elle ne lit que le sexe, l'âge, le poids et la taille : quatre entrées certaines. |  |
| 9 | `lib/methodologie.ts:80` | corps | Un taux de masse grasse choisi sur une silhouette porte une marge de l'ordre de ±5 points. Quand il indique nettement plus de masse maigre que la moyenne du gabarit, au-delà de ce bruit, le calcul glisse progressivement vers Katch-McArdle, qui repose sur cette masse maigre. | ⚑ |
| 10 | `lib/methodologie.ts:81` | corps | Jamais l'inverse : si la formule à masse maigre donne une dépense plus basse, c'est Mifflin-St Jeor qui reste servie. |  |
| 11 | `lib/methodologie.ts:82` | corps | Katch-McArdle n'est utilisée telle quelle que dans un seul cas : un taux MESURÉ (impédancemétrie, DEXA, plis cutanés) et déclaré comme tel. La question n'est posée qu'au-delà de 35 % (homme) et 43 % (femme) : en dessous, le taux est toujours traité comme une estimation. | ⚑ |
| 12 | `lib/methodologie.ts:83` | corps | Le facteur d'activité hors sport va de 1,3 (travail assis) à 1,45 (métier physique). La table s'arrête volontairement à 1,45 : les valeurs plus hautes des tables classiques incluent l'exercice, qui est déjà compté à part. | ⚑ |
| 13 | `lib/methodologie.ts:84` | corps | La dépense des séances est calculée par la méthode des équivalents métaboliques (MET), en valeur NETTE : le métabolisme de repos de l'heure de séance est retiré, parce qu'il est déjà compté par les deux premiers termes. |  |
| 14 | `lib/methodologie.ts:105` | titre | La répartition des macronutriments |  |
| 15 | `lib/methodologie.ts:107` | corps | La cible protéique dépend de l'objectif et se calcule sur un poids ajusté à la composition corporelle. Elle est ensuite bornée entre 1,6 et 2,6 g par kg de MASSE MAIGRE, quelle que soit la corpulence. | ⚑ |
| 16 | `lib/methodologie.ts:108` | corps | Les lipides ne descendent jamais sous 0,8 g par kg de poids de corps, seuil en deçà duquel l'apport en acides gras essentiels et l'absorption des vitamines liposolubles ne sont plus assurés. | ⚑ |
| 17 | `lib/methodologie.ts:109` | corps | Les glucides reçoivent le budget restant. |  |
| 18 | `lib/methodologie.ts:130` | titre | Les limites de sécurité |  |
| 19 | `lib/methodologie.ts:143` | corps | Ces limites ne sont pas des réglages : le code les applique à chaque calcul, quel que soit l'objectif choisi ou la date visée. |  |
| 20 | `lib/methodologie.ts:144` | corps | Deux d'entre elles sont infranchissables CHAQUE JOUR : le métabolisme de base et le filet absolu. L'énergie disponible, elle, se juge sur la SEMAINE : c'est une moyenne soutenue, et c'est ainsi que la littérature la définit. |  |
| 21 | `lib/methodologie.ts:145` | corps | Le budget d'un jour suit la dépense de ce jour-là : un jour de séance reçoit plus qu'un jour de repos, et la semaine conserve son total. Un jour calme peut donc passer sous le seuil d'énergie disponible sans que la semaine y passe , et le métabolisme de base, lui, reste un plancher quotidien. |  |
| 22 | `lib/methodologie.ts:148` | corps | Énergie disponible : au moins 30 kcal par kg de masse maigre, une fois la dépense sportive retirée. Ce plancher existe parce que la littérature documente, en dessous, des perturbations hormonales et osseuses (déficit énergétique relatif dans le sport, RED-S) : c'est ce que Kyroz tient à distance. | ⚑ |
| 23 | `lib/methodologie.ts:149` | corps | Au-delà de 12 semaines cumulées en zone basse, c'est-à-dire entre 30 et 35 kcal par kg de masse maigre, ce plancher remonte progressivement vers 35 : l'app force une sortie de déficit au lieu de la laisser durer. | ⚑ |
| 24 | `lib/methodologie.ts:150` | corps | Filet absolu : jamais moins de 1 500 kcal par jour chez l'homme et 1 200 kcal chez la femme. | ⚑ |
| 25 | `lib/methodologie.ts:151` | corps | Déficit plafonné à 25 % de la dépense estimée. | ⚑ |
...
| 27 | `lib/methodologie.ts:158` | corps | Sous un indice de masse corporelle de départ de 18,5, Kyroz ne creuse aucun déficit et sert un plan complet à la maintenance, de même que pour tout poids cible sortant de la plage saine. | ⚑ |
...
| 33 | `lib/methodologie.ts:198` | corps | Tout ce qui précède ne sort pas de la littérature au même titre. Les deux listes ci-dessous rangent chaque valeur citée sur cette page : un chiffre qui n'y figurerait pas serait un oubli, pas une omission volontaire. |  |
| 34 | `lib/methodologie.ts:199` | corps | Viennent de la littérature : les deux équations de métabolisme de base et les valeurs MET · le seuil de 30 kcal par kg de masse maigre et son optimum à 35 · le plancher lipidique de 0,8 g par kg de poids de corps · les fourchettes protéiques, de 1,6 à 2,6 g par kg de masse maigre · l'indice de masse corporelle de 18,5 sous lequel un déficit n'a plus de sens · l'âge de 18 ans en deçà duquel ces équations ne sont pas validées. | ⚑ |
| 35 | `lib/methodologie.ts:200` | corps | Sont des choix de Kyroz, prudents par construction : la table d'activité quotidienne, de 1,3 à 1,45, et son plafond · le déficit borné à 25 % · le filet absolu de 1 500 et 1 200 kcal · la pause à la maintenance toutes les 8 semaines · la remontée du plancher après 12 semaines cumulées en zone basse. | ⚑ |
| 36 | `lib/methodologie.ts:201` | corps | Sont aussi des choix de Kyroz, et ce sont ceux qui s'écartent le plus de la littérature : le glissement de Mifflin-St Jeor vers Katch-McArdle, la marge de ±5 points qui définit ce « nettement plus de masse maigre », et le seuil de 35 % (homme) et 43 % (femme) au-delà duquel la provenance du taux est demandée · le retrait progressif des planchers dérivés de la masse maigre au-delà de 30 % (homme) et 40 % (femme) de masse grasse : au-delà, la réserve adipeuse est la source d'énergie que ces planchers, conçus pour des athlètes maigres, interdisaient d'utiliser. | ⚑ |
| 37 | `lib/methodologie.ts:202` | corps | Une estimation de dépense reste une estimation : elle porte une marge d'erreur individuelle que ces équations ne suppriment pas. Le poids relevé au fil des semaines est le seul juge, et c'est lui que Kyroz suit. |  |
