# B10 — 15 petits-dejeuners du quotidien francais, format genereux
...
**15 recettes de catégorie `petit_dej` (petit-déjeuner).**

ids à produire, dans cet ordre, sans trou et sans doublon :
`pd123`, `pd124`, `pd125`, `pd126`, `pd127`, `pd128`, `pd129`, `pd130`, `pd131`, `pd132`, `pd133`, `pd134`, `pd135`, `pd136`, `pd137`
...
| | Nombre |
|---|---|
| **Carnées ou marines** — contiennent viande, volaille ou poisson | **5** |
| **Végétariennes** — œufs et/ou laitages, **aucune** chair animale | **10** |
| **Vegan** — aucun produit animal (ni œuf, ni laitage, ni miel) | **0** |
| **dont sans gluten, toutes lignes confondues** | **≥ 3** |

Les seules ancres carnées que le §4 t'autorise sur ce créneau : `dinde_escalope` · `jambon_blanc` · `saumon_fume` · `thon_naturel`. Aucune ne peut porter plus de 3 recettes (§7).
...
|---|---|---|
| 413 kcal · 24 g P | 9 / 12 |
| 470 kcal · 28 g P | 12 / 12 |
| 523 kcal · 31 g P | 12 / 12 | ← l'enveloppe de CE lot
| 533 kcal · 32 g P | 12 / 12 | ← l'enveloppe de CE lot
| 575 kcal · 35 g P | 12 / 12 | ← l'enveloppe de CE lot
| 638 kcal · 39 g P | 11 / 12 | ← l'enveloppe de CE lot
| 721 kcal · 44 g P | 11 / 12 |
...
> **Densité protéique imposée : 5.0 à 8.5 g de protéines pour 100 kcal.**
...
|---|---|---|---|---|---|---|---|---|---|
| `amandes` | Amandes | g | 631 | 21.4 | 8.8 | 52.5 | — | ≤ 40 | 17 |
| `ananas` | Ananas | g | 52 | 0.2 | 11.7 | 0.5 | — | — | 1 |
| `asperges` | Asperges | g | 25 | 2.5 | 2 | 0.3 | — | — | 1 |
| `avocat` | Avocat | g | 203 | 1.6 | 0 | 20.6 | — | ≤ 100 | 10 |
| `banane` | Banane | g | 88 | 1.1 | 19.7 | 0.5 | — | — | 21 |
| `beurre_amande` | Beurre d'amande | g | 631 | 21.4 | 8.8 | 52.5 | — | ≤ 40 | 7 |
...
| `brocoli` | Brocoli | g | 32 | 2.9 | 2.1 | 0.4 | — | — | 1 |
| `cacao_poudre` | Cacao maigre en poudre | g | 387 | 22.4 | 11.6 | 20.6 | — | — | 10 |
| `carotte` | Carotte | g | 30 | 0.8 | 5.2 | 0.5 | — | — | 1 |
| `champignons` | Champignons | g | 21 | 2.1 | 1.8 | 0.4 | — | — | 9 |
| `chataigne` | Châtaigne | g | 189 | 2 | 36.8 | 1.8 | — | — | 6 |
...
| `courgette` | Courgette | g | 17 | 1.2 | 1.8 | 0.3 | — | — | 1 |
| `creme_soja` | Crème de soja | ml | 152 | 3.2 | 2 | 14.7 | — | ≤ 80 | 2 |
| `dattes` | Dattes dénoyautées | g | 287 | 1.8 | 64.7 | 0.2 | — | ≤ 60 | 3 |
| `dinde_escalope` | Escalope de dinde | g | 108 | 23.7 | 0 | 1.5 | cru | — | 4 |
| `edamame` | Edamame | g | 125 | 11 | 9 | 5 | — | — | 2 |
| `epinards` | Épinards frais | g | 33 | 2.7 | 3.1 | 0.4 | — | — | 17 |
...
| `galette_riz` | Galette de riz soufflé | g | 381 | 8.4 | 77.9 | 3 | — | — | 4 |
| `graines_chia` | Graines de chia | g | 454 | 16.5 | 7.7 | 30.7 | — | ≤ 35 | 14 |
| `graines_courge` | Graines de courge | g | 618 | 29.5 | 5.4 | 49.1 | — | ≤ 30 | 14 |
...
| `millet` | Millet | g | 360 | 11 | 73 | 4 | SEC | — | 10 |
| `myrtilles` | Myrtilles | g | 58 | 0.9 | 10.6 | 0.3 | — | — | 8 |
| `noisettes` | Noisettes | g | 632 | 14.4 | 7.2 | 56.9 | — | ≤ 35 | 12 |
| `noix` | Noix | g | 709 | 13.3 | 6.9 | 67.3 | — | ≤ 35 | 10 |
| `nouilles_riz` | Nouilles de riz | g | 365 | 7.4 | 80.5 | 1 | SEC | — | 1 |
| `oeuf_entier` | Œuf entier | g | 140 | 12.8 | 0.1 | 9.8 | — | — | 16 |
| `oignon` | Oignon | g | 39 | 1.1 | 6.2 | 0.6 | — | — | 1 |
| `pain_complet` ⛔SG | Pain complet | g | 234 | 8.7 | 41.2 | 1.7 | — | — | 11 |
...
| `pois_chiches_conserve` | Pois chiches (conserve, égouttés) | g | 122 | 6.7 | 15 | 2.7 | — | — | 2 |
| `poivron` | Poivron | g | 23 | 0.8 | 3.5 | 0.3 | — | — | 8 |
...
**78 de ces 87 refs sont compatibles sans gluten** (ceux sans ⛔SG).

---

## 5. Format de sortie exact

Un seul objet JSON, une seule clé `recipes`, 15 objets. Pas de `_meta`, pas de
`config`, pas de commentaire dans le JSON.

```json
{ "recipes": [ /* les 15 recettes */ ] }
```
...
| `category` | `"petit_dej"` pour les 15 |
...
| `instructions` | **2 à 4 étapes** — voir §6 |
...
| `wave` | `"2026-09-07-b10-registre-francais"` pour les 15 — c'est le nom du dossier de drop qui portera cette livraison, la convention du catalogue (`_meta.waves`). Recopie-le tel quel. |
...
Ces contrôles s'appliquent aussi **entre les 15 recettes de ce lot**.
...
Aucun `ref` protéique ne peut porter plus de **25 % des 15 recettes**, soit
...
- `huile_olive` (fat) — déjà dans 20 recettes
- `yaourt_soja_proteine` (protein) — déjà dans 20 recettes
- `amandes` (fat) — déjà dans 17 recettes
...
- `graines_chia` (fat) — déjà dans 14 recettes
- `graines_courge` (fat) — déjà dans 14 recettes
- `skyr` (protein) — déjà dans 13 recettes
- `noisettes` (fat) — déjà dans 12 recettes
- `pain_complet` (carb) — déjà dans 11 recettes
- `polenta` (carb) — déjà dans 11 recettes
- `sarrasin` (carb) — déjà dans 11 recettes
- `avocat` (fat) — déjà dans 10 recettes
...
protéiques par ordre de **disponibilité** — **toutes prises dans les 87 refs du §4**,
...
|---|---|---|---|
| `haricots_rouges_conserve` | — | — | 22 |
| `thon_naturel` | — | — | 22 |
| `lentilles_cuites` | 1 | — | 22 |
| `edamame` | 2 | — | 22 |
| `seitan` | 2 | — | 22 |
| `soja_texture` | 2 | — | 22 |
| `tempeh` | 3 | — | 22 |
...
| `yaourt_grec` | 2 | 1 | 21 |
| `tofu_ferme` | 6 | 1 | 21 |
| `cottage_cheese` | 8 | 1 | 21 |
| `whey` | 15 | 1 | 21 |
| `fromage_blanc_0` | 6 | 2 | 20 |

**Refs du §4 JAMAIS employés en `petit_dej`** — terrain entièrement vierge, aucun risque de
doublon :

`boisson_soja` · `falafel` · `haricots_rouges_conserve` · `thon_naturel`
...
## 8. Annexe — les 122 recettes `petit_dej` déjà au catalogue
...
Refs les plus fréquents sur ce créneau (à éviter de renforcer) : `flocons_avoine` 23 · `banane` 21 · `huile_olive` 20 · `yaourt_soja_proteine` 20 · `amandes` 17 · `epinards` 17 · `proteine_vegetale` 17 · `tomate` 17 · `lait_amande` 16 · `oeuf_entier` 16 · `whey` 15 · `graines_chia` 14 · `graines_courge` 14 · `skyr` 13 · `noisettes` 12 · `pomme` 12 · `lait_demi_ecreme` 11 · `pain_complet` 11 · `sarrasin` 11 · `polenta` 11 · `framboises` 10 · `avocat` 10 · `cacao_poudre` 10 · `noix` 10 · `millet` 10 · `champignons` 9 · `beurre_cacahuete` 9 · `myrtilles` 8 · `blanc_oeuf` 8 · `fruits_rouges` 8 · `mangue` 8 · `cottage_cheese` 8 · `poivron` 8 · `quinoa` 8 · `pain_seigle` 7 · `beurre_amande` 7 · `fromage_blanc_0` 6 · `tofu_ferme` 6 · `concombre` 6 · `lait_coco` 6 · `chataigne` 6 · `miel` 5 · `saumon_fume` 5 · `levure_maltee` 5 · `patate_douce` 5

| id | ensemble de refs |
|---|---|
| pd01 | `whey` · `lait_demi_ecreme` · `flocons_avoine` · `banane` · `myrtilles` · `amandes` |
| pd02 | `flocons_avoine` · `skyr` · `graines_chia` · `framboises` · `lait_amande` |
| pd03 | `flocons_avoine` · `banane` · `oeuf_entier` · `blanc_oeuf` · `whey` · `noisettes` |
| pd04 | `oeuf_entier` · `jambon_blanc` · `champignons` · `huile_olive` · `pain_complet` |
| pd05 | `oeuf_entier` · `avocat` · `pain_complet` · `epinards` |
| pd06 | `skyr` · `flocons_avoine` · `amandes` · `fruits_rouges` · `miel` |
| pd07 | `proteine_vegetale` · `mangue` · `banane` · `epinards` · `lait_amande` · `flocons_avoine` · `amandes` |
...
| pd11 | `blanc_oeuf` · `oeuf_entier` · `epinards` · `feta` · `huile_olive` · `pain_complet` |
| pd12 | `pain_complet` · `oeuf_entier` · `lait_demi_ecreme` · `whey` · `fruits_rouges` · `sirop_erable` · `amandes` |
| pd13 | `tortilla_complete` · `oeuf_entier` · `dinde_escalope` · `poivron` · `avocat` |
...
| pd18 | `pain_pita_complet` · `saumon_fume` · `cottage_cheese` · `concombre` · `avocat` |
| pd19 | `whey` · `flocons_avoine` · `lait_demi_ecreme` · `dattes` · `beurre_cacahuete` · `banane` |
| pd20 | `oeuf_entier` · `tomate_concassee` · `poivron` · `pois_chiches_conserve` · `huile_olive` · `pain_complet` |
...
| pd37 | `blanc_oeuf` · `oeuf_entier` · `tomate` · `pain_seigle` · `huile_olive` |
| pd38 | `oeuf_entier` · `avocat` · `pain_seigle` · `tomate` |
| pd39 | `flocons_avoine` · `whey` · `blanc_oeuf` · `myrtilles` · `noix` |
| pd40 | `pain_complet` · `cottage_cheese` · `saumon_fume` · `concombre` |
| pd41 | `pain_complet` · `beurre_cacahuete` · `banane` · `yaourt_soja_proteine` |
| pd42 | `pain_pita_complet` · `pois_chiches_conserve` · `oeuf_entier` · `tomate` · `huile_olive` |
...
| pd46 | `millet` · `proteine_vegetale` · `lait_amande` · `cacao_poudre` · `dattes` · `noisettes` |
...
| pd60 | `polenta` · `lait_amande` · `proteine_vegetale` · `cacao_poudre` · `banane` · `noix` |
...
| pd64 | `riz_basmati` · `lait_demi_ecreme` · `whey` · `framboises` · `amandes` |
...
| pd68 | `oeuf_entier` · `banane` · `skyr` · `framboises` · `amandes` |
...
| pd111 | `tempeh` · `sarrasin` · `graines_courge` · `epinards` · `tomate` |
| pd112 | `edamame` · `millet` · `graines_chia` · `poivron` |
| pd113 | `soja_texture` · `polenta` · `lait_coco` · `champignons` |
| pd114 | `tofu_ferme` · `quinoa` · `creme_soja` · `courgette` |
| pd115 | `tofu_fume` · `nouilles_riz` · `creme_soja` · `legumes_wok` |
| pd116 | `tempeh` · `riz_complet` · `huile_olive` · `brocoli` |
| pd117 | `edamame` · `galette_riz` · `beurre_amande` · `carotte` |
| pd118 | `soja_texture` · `chataigne` · `lait_coco` · `haricots_verts` |
| pd119 | `tofu_ferme` · `millet` · `graines_courge` · `asperges` |
| pd120 | `seitan` · `semoule_couscous` · `huile_olive` · `poivron` · `oignon` |
| pd121 | `seitan` · `pain_complet` · `tahini` · `champignons` |
| pd122 | `tempeh` · `pain_pita_complet` · `huile_olive` · `salade_verte` · `tomate` |
...
- [ ] `wave` = `"2026-09-07-b10-registre-francais"` sur les 15.
