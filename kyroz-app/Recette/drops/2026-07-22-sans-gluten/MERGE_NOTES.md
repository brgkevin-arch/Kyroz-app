# Notes de merge — push catalogue Kyroz (+214 recettes)

## Ce que contient le livrable
- **`recettes-kyroz-nouvelles.json`** (214 recettes, dont les 50 sans gluten)
- **`recettes-sans-gluten.json`** : les 50 nouvelles recettes GF isolées (pour relecture ; déjà incluses dans le fichier ci-dessus — **ne pas merger deux fois**)
- (ancien) : un *fragment* à fusionner, avec deux clés :
  - `ingredients_reference` → **11 nouveaux ingrédients** à ajouter à la table existante.
  - `recipes` → **214 nouvelles recettes** à concaténer au tableau `recipes` existant.
...
5. **Compteur de test** : passer `expect(RECIPES).toHaveLength(100)` → **`314`**.
