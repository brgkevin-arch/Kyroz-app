...
## Data / thème / qualité
- **Base d'aliments** (`lib/foods.ts`, type `Food`) — **Phase 1 livrée** : ~118 aliments soignés (valeurs /100 g alignées Ciqual, formes cuites), `searchFoods` (recherche libre insensible accents/casse, classée préfixe>mot>sous-chaîne), `macrosFromIngredients`/`recipeMacrosPerPortion` (recalcul des macros depuis les ingrédients liés via `Ingredient.food_id`). **Phase 2 livrée** : le `RecipeEditor` lie les ingrédients à la base (recherche par ligne → `food_id`), toggle macros **Auto (calculées depuis les ingrédients)** / **Manuel** (repli). En Auto, `macros_per_portion` = `recipeMacrosPerPortion` au save → le plan suit. Non-breaking : les 50 recettes de base (sans `food_id`) s'ouvrent en Manuel, inchangées. **Décisions** : approche A (moyenne) d'abord, B (fourchette) après tests utilisateurs ; honnêteté `± kcal` sur le total du jour (Phase 3). **RESTE** : Phase 3 = marge ± kcal (via `Food.uncertainty_pct`) + **import Ciqual complet (~3000)** qui REMPLACERA les ~118 aliments manuels (fichier ANSES à poser dans le repo).
...
- **Tests** : **116 / 10 fichiers** (`npm test`, vitest) — garde-fous §6, masse maigre, mode percent, fuseau, déterminisme, recalage du jour, fibres, courses→frigo, units (régression bœuf), intégrité 50 recettes. **Error Boundary** global (`app/_layout.tsx`).
