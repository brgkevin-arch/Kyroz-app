...
## Data / thème / qualité
- **Base d'aliments** (`lib/foods.ts`, type `Food`) — **Phase 1 livrée** : ~118 aliments soignés (valeurs /100 g alignées Ciqual, formes cuites), `searchFoods` (recherche libre insensible accents/casse, classée préfixe>mot>sous-chaîne), `macrosFromIngredients`/`recipeMacrosPerPortion` (recalcul des macros depuis les ingrédients liés via `Ingredient.food_id`). ⚠️ Aujourd'hui les macros de recette sont **tapées à la main** et déconnectées des ingrédients — le moteur ci-dessus n'est PAS encore branché dans l'UI. **Décisions** : approche A (moyenne) d'abord, B (fourchette) après tests utilisateurs ; honnêteté `± kcal` sur le total du jour (Phase 3). **RESTE** : Phase 2 = brancher `RecipeEditor` (recherche d'aliment par ingrédient + recalcul live) ; Phase 3 = marge ± kcal + import Ciqual complet (~3000, fichier ANSES à poser dans le repo). `uncertainty_pct` sur `Food` prévu pour la marge.
